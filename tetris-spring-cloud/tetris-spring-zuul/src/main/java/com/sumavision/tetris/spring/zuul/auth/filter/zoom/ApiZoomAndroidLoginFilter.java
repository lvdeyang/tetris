package com.sumavision.tetris.spring.zuul.auth.filter.zoom;
 ...
 */
public class ApiZoomAndroidLoginFilter extends ZuulFilter{

	private String[] ignores = new String[]{
		"/api/zoom/android/auth/do/username/password/login",
		"/api/zoom/android/*"
	};
	...
	public Object run() {
		...
		try{
			boolean result = tokenQuery.checkToken(token, TerminalType.ZOOM_ANDROID);
			if(!result){
	...
	public boolean shouldFilter() {
		...
		String requestUri = request.getRequestURI();
		requestUri = requestUri.replace(new StringBufferWrapper().append("/").append(requestUri.split("/")[1]).toString(), "");
		if(requestUri.startsWith(TerminalType.ZOOM_ANDROID.getUriPrefix())){
			return true;
		}else{
			return false;
		}
