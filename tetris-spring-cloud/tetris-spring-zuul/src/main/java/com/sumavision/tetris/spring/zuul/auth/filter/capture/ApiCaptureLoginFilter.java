package com.sumavision.tetris.spring.zuul.auth.filter.capture;
 ...
 */
public class ApiCaptureLoginFilter extends ZuulFilter{
	...
	public boolean shouldFilter() {
		...
		String requestUri = request.getRequestURI();
		requestUri = requestUri.replace(new StringBufferWrapper().append("/").append(requestUri.split("/")[1]).toString(), "");
		if(requestUri.startsWith("/api/capture")){
			return true;
		}else{
			return false;
		}
