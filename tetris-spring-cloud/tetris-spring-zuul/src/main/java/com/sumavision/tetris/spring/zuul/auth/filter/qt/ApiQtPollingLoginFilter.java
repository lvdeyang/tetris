 ...
 * <b>作者:</b>lzp<br/>
 * <b>版本：</b>1.0<br/>
 * <b>日期：</b>2019年12月12日 下午1:34:26
 */
public class ApiQtPollingLoginFilter extends ZuulFilter{
	...
	public Object run() {
		...
		try{
			boolean result = tokenQuery.checkToken(token, TerminalType.QT_POLLING);
			if(!result){
				ctx.setResponseStatusCode(403);
				ctx.setSendZuulResponse(false);
			}
	...
	public boolean shouldFilter() {
		...
		String requestUri = request.getRequestURI();
		requestUri = requestUri.replace(new StringBufferWrapper().append("/").append(requestUri.split("/")[1]).toString(), "");
		if(requestUri.startsWith(TerminalType.QT_POLLING.getUriPrefix())){
			return true;
		}else{
			return false;
		}
