 ...
 */
public class ApiZoomQtLoginFilter extends ZuulFilter{

	private String[] ignores = new String[]{
		"/api/zoom/qt/auth/do/username/password/login"
	};
	...
	public Object run() {
		...
		try{
			boolean result = tokenQuery.checkToken(token, TerminalType.ZOOM_QT);
			if(!result){
	...
	public boolean shouldFilter() {
		...
		String requestUri = request.getRequestURI();
		requestUri = requestUri.replace(new StringBufferWrapper().append("/").append(requestUri.split("/")[1]).toString(), "");
		if(requestUri.startsWith(TerminalType.ZOOM_QT.getUriPrefix())){
			return true;
		}else{
			return false;
		}
