  ...
  Vue.component(pluginName, {
    ...
    methods: {
      ...
      getCommandList: function () {
        var self = this;
        ajax.post('/command/basic/query/members/list', { id: self.group.current.id }, function (data) {
          self.specialCommand = data.specialCommand;
        });
      },
      ...
      secretCommand: function () {
        ...
        ajax.post('/command/secret/start', { id: self.group.current.id, userId: userId }, function (data) {
      ...
      resetNameClose: function () {
        ...
        self.dialog.resetName.temporary = false;
        self.cancelChoose();
      },
        ...
        if (self.dialog.resetName.createType == 1) {  //指挥
          if (self.dialog.resetName.type === 'new') {
            ...
            ajax.post('/command/basic/save', {
              ...
              temporary: self.dialog.resetName.temporary
            }, function (data) {
              data.type = 'command';
              data.select = [];
              self.group.current = data;
      ...
      currentGroupChange: function (v, donotRefreshCommand) {
        ...
        if (typeof v === 'object') {
          ...
          var _fn = function () {
            ...
            // 停止特殊指挥按钮显示什么，由以下接口返回specialCommand决定
      ...
      cooperativeCommand: function (name) {
        ...
        if (self.differentiate === 1) {
          ...
          // }, { width: 980, height: 600 });
          var userIds = [];
          self.group.current.select.forEach(function (item) {
            userIds.push(item.id)
          })
          if (userIds.length == 0) {
            self.$message.error("请至少选择一个成员！")
            return
          }
      ...
      // 越级指挥
      crossCommand: function () {
        var self = this;
        var userIds = [];
        if (self.group.current.select.length !== 2) {
          self.$message.error("请选择两个个成员！")
          return
        }
        self.group.current.select.forEach(function (item) {
          userIds.push(item.id)
        })
        ajax.post('/command/cross/grant', {
          id: self.group.current.id,
          userIds: $.toJSON(userIds)
        }, function (data) {
      ...
      // 授权指挥
      authorizeCommand: function () {
        var self = this;
        var userIds = [];
        if (self.group.current.select.length !== 1) {
          self.$message.error("接收成员不能为空，且只能有一个！")
          return
        }
        self.group.current.select.forEach(function (item) {
          userIds.push(item.id)
        })
        ajax.post('/command/authorize/grant', {
          id: self.group.current.id,
          acceptId: userIds[0]
        }, function (data) {
      ...
      cancelCooperativeCommand: function () {
        ...
        switch (this.specialCommand) {
          case 'SECRET':
            url = '/command/secret/stop';
            break
          case 'COOPERATE':
            url = '/command/cooperation/stop';
            break
          case 'CROSS':
            url = '/command/cross/stop';
            break
          case 'REPLACE':
            url = '/command/replace/stop';
            break
          case 'AUTHORIZE':
            url = '/command/authorize/stop';
            break
        }
        ajax.post(url, {
          id: self.group.current.id
        }, function (data) {
          self.specialCommand = null;
        }, null, [403]);
      },
      //会议成员
          ...
          if (self.user.id == self.group.current.creator) {
            ajax.post('/command/basic/stop', {
              ...
              if (self.group.entered.length > 0) {
                self.group.current = self.group.entered[0];
                self.group.currentId = String(self.group.entered[0].id);
                self.currentGroupChange(self.group.currentId);
              } else {
                // self.group.current = '';
                // self.group.currentId = '';
    ...
    computed: {
      ...
      currentSpecialCommand: function () {
    ...
    mounted: function () {
      ...
      self.qt = new QtContext('rightBar', function () {
        ...
        self.qt.on('getCommandListFun', function (e) {
          self.getCommandList();
        });
      });
    }
