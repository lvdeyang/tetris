 ...
 */
define([
    'text!' + window.APPPATH + 'tetris/model/agenda/page-custom-agenda.html',
    'config',
    'jquery',
    'restfull',
    'context',
    ...
    'element-ui',
    'mi-frame',
    'css!' + window.APPPATH + 'tetris/model/agenda/page-custom-agenda.css',
    ...
    var init = function(p){
        ...
        vueInstance = new Vue({
            ...
            computed:{
                ...
                dialogSetLoopCurrentTab:function(){
                    var self = this;
                    return self.dialog.setLoop.currentTab;
                }
            },
            watch:{
                ...
                dialogSetLoopCurrentTab:function(val){
                    var self = this;
                    self.$nextTick(function(){
                        if(val === 'member'){
                            self.$refs.setLoopMemberTree.filter('VIDEO_ENCODE');
                        }else if(val === 'role'){
                            self.$refs.setLoopRoleTree.filter('VIDEO_ENCODE');
                        }
                    });
                }
            },
            methods:{
            	gotoCustomVirtualSource:function(){
                    var self = this;
                    window.location.hash = '#/page-combine-virtual-source/' + self.groupId + '/' + self.groupName;
                },
                handleDropdownCommand:function(cmd){
                    var self = this;
                    if(cmd === 'editParam'){
                        self.handleEditParam();
                    }else if(cmd === 'publishStream'){
                        self.handlePublishStream();
                    }
                },
                ...
                handleDrop:function(e, position){
                    ...
                    var sourceType = 'GROUP_MEMBER_CHANNEL';
                    if(nodeData.type==='ROLE_CHANNEL'){
                        sourceType = 'ROLE_CHANNEL';
                    }
                    if(nodeData.channelType==='COMBINE_VIDEO_VIRTUAL_SOURCE'){
                        sourceType = 'COMBINE_VIDEO_VIRTUAL_SOURCE';
                    }
                ...
                loadVirtualSource:function(){
                    var self = this;
                    self.roles.data.splice(0, self.roles.data.length);
                    ajax.post('/tetris/bvc/model/virtual/source/load', {
                        businessId:self.groupId
                    }, function(data){
                        var virtualSources = data.virtualSources;
                        var virtualSourceFolder = {
                            id:-111,
                            name:'虚拟源',
                            type:'COMBINE_VIDEO_VIRTUAL_SOURCE_FOLDER',
                            isLeaf:false,
                            children:[]
                        };
                        if(virtualSources && virtualSources.length>0){
                            self.virtualSource.data.push(virtualSourceFolder);
                            for(var i=0; i<virtualSources.length; i++) {
                                var virtualSource = virtualSources[i];
                                var channelNode = {
                                    id: virtualSource.id,
                                    name: virtualSource.name,
                                    type: 'ROLE_CHANNEL',
                                    channelType: "COMBINE_VIDEO_VIRTUAL_SOURCE",
                                    channelTypeName: virtualSource.typeName,
                                    isLeaf: true
                                };
                                virtualSourceFolder.children.push(channelNode);
                            }
                        }
                        self.$nextTick(function(){
                            self.$refs.virtualSourceTree.filter('__handle_init_');
                        });
                    });
                },
                filterNodeByName:function(val, data, node){
                    if(data.channelType && data.channelType.indexOf('DECODE')>=0){
                        return false;
                    }
                    ...
                    if(data.type==='ROLE_CHANNEL' && data.channelType === 'VIDEO_ENCODE'){
                        if(node.parent.parent.data.type=='ONE_TO_MANY_FOLDER'){
                            return false;
                        }
                    }
                ...
                allowDrag:function(draggingNode){
                    if(draggingNode.data.type==='ROLE_CHANNEL' && draggingNode.parent.parent.data.type=='ONE_TO_MANY_FOLDER'){
                        return false;
                    }
                    ...
                    if((draggingNode.data.type==='ROLE_CHANNEL' && draggingNode.data.channelType==='VIDEO_ENCODE')
                        ...
                        || (draggingNode.data.type==='ROLE_CHANNEL' && draggingNode.data.channelType==='COMBINE_VIDEO_VIRTUAL_SOURCE')){
                        return true;
                    }
                    return false;
                },
                ...
                loadAgendas:function(){
                    ...
                    ajax.post('/tetris/bvc/model/agenda/load/by/business/id/and/business/info/type', {
                        businessId:self.groupId
                        //businessInfoType:'BASIC_MEETING'
                    }, function(data){
                        if(data && data.length>0){
                            for(var i=0; i<data.length; i++){
                ...
                handleAddForwardSubmit:function(agendaId){
                    var self = this;
                ...
                filterNode:function(val, data){
                    ...
                    if(val === 'AUDIO_ENCODE'){
                        ...
                        if((data.channelType && data.channelType.indexOf('COMBINE_VIDEO_VIRTUAL_SOURCE')>=0) ||
                            (data.channelType && data.channelType.indexOf('AUDIO_')>=0) ||
                ...
                handleSetLoop:function(position){
                    ...
                    self.dialog.setLoop.members.members.splice(0, self.dialog.setLoop.members.members.length);
                    self.dialog.setLoop.members.roles.splice(0, self.dialog.setLoop.members.roles.length);
                    for(var i=0; i<self.members.data.length; i++){
                        self.dialog.setLoop.members.members.push(self.members.data[i]);
                    }
                    for(var i=0; i<self.roles.data.length; i++){
                        if(self.roles.data[i].id === -2) continue;
                        self.dialog.setLoop.members.roles.push(self.roles.data[i]);
                    }
                    if(self.forward.current.sources && self.forward.current.sources.length>0){
                        for(var i=0; i<self.forward.current.sources.length; i++){
                ...
                handleSetLoopClose:function(){
                    ...
                    self.$refs.setLoopMemberTree.setCurrentNode([]);
                    self.$refs.setLoopRoleTree.setCurrentNode([]);
                    self.dialog.setLoop.members.members.splice(0, self.dialog.setLoop.members.members.length);
                    self.dialog.setLoop.members.roles.splice(0, self.dialog.setLoop.members.roles.length);
                ...
                handleSetLoopSelect:function(){
                    var self = this;
                    var checkedMemberNodeDatas = self.$refs.setLoopMemberTree.getCheckedNodes();
                    var checkedRoleNodeDatas = self.$refs.setLoopRoleTree.getCheckedNodes();
                    if(checkedMemberNodeDatas && checkedMemberNodeDatas.length>0){
                        for(var i=0; i<checkedMemberNodeDatas.length; i++){
                            if((checkedMemberNodeDatas[i].type==='ROLE_CHANNEL' && checkedMemberNodeDatas[i].channelType==='VIDEO_ENCODE')||
                                (checkedMemberNodeDatas[i].type==='CHANNEL'  && checkedMemberNodeDatas[i].param.indexOf('VIDEO_ENCODE')>=0)){
                                if(checkedMemberNodeDatas[i].visible){
                                    var node = self.$refs.setLoopMemberTree.getNode(checkedMemberNodeDatas[i]);
                                    var parentNode = node.parent;
                                    self.dialog.setLoop.selected.push({checked:false, node:checkedMemberNodeDatas[i], parent:parentNode.data});
                                }
                            }
                        }
                        self.$refs.setLoopMemberTree.filter('VIDEO_ENCODE');
                    ...
                    if(checkedRoleNodeDatas && checkedRoleNodeDatas.length>0){
                        for(var i=0; i<checkedRoleNodeDatas.length; i++){
                            if((checkedRoleNodeDatas[i].type==='ROLE_CHANNEL' && checkedRoleNodeDatas[i].channelType==='VIDEO_ENCODE')||
                                (checkedRoleNodeDatas[i].type==='CHANNEL'  && checkedRoleNodeDatas[i].param.indexOf('VIDEO_ENCODE')>=0)){
                                if(checkedRoleNodeDatas[i].visible){
                                    var node = self.$refs.setLoopRoleTree.getNode(checkedRoleNodeDatas[i]);
                                    var parentNode = node.parent;
                                    self.dialog.setLoop.selected.push({checked:false, node:checkedRoleNodeDatas[i], parent:parentNode.data});
                                }
                            }
                        }
                        self.$refs.setLoopRoleTree.filter('VIDEO_ENCODE');
                ...
                handlePublishStream:function(){
                    var self = this;
                    self.dialog.publishStream.visible = true;
                },
                handlePublishStreamClose:function(){
                    var self = this;
                    self.dialog.publishStream.visible = false;
                    self.dialog.publishStream.loading = false;
                    self.dialog.publishStream.id = '';
                    self.dialog.publishStream.rtmpUrl = '';
                },
                handlePublishStreamSubmit:function(){
                    var self = this;
                    self.dialog.publishStream.loading = true;
                    ajax.post('/command/record/start/publishment', {
                        groupId:self.group.id,
                        rtmpUrl:self.dialog.publishStream.rtmpUrl
                    }, function(data, status , message){
                        ...
                        self.group.isPublishment = true;
                        self.handlePublishStreamClose();
                    }, null, ajax.TOTAL_CATCH_CODE);
                },
