...
public class GroupProceedRecordController {

	@Autowired
	private GroupProceedRecordDAO groupProceedRecordDao;

	@Autowired
	private GroupProceedRecordQueryServiceImpl groupProceedRecordQueryServiceImpl;
	...
	public Object queryRecords(
		...
		Page<GroupProceedRecordPO> records = groupProceedRecordDao.findByGroupIdOrderByStartTimeDesc(groupId, page);
		long total = records.getTotalElements();
		List<GroupProceedRecordVO> vos = new ArrayList<GroupProceedRecordVO>();
		for(GroupProceedRecordPO record : records.getContent()){
			vos.add(new GroupProceedRecordVO().set(record));
		}		
	...
	public Object statisticsTotalOfRecordTimePeople(
		...
		return groupProceedRecordQueryServiceImpl.statisticsTimeAndNumberOfPeopleAndNumberOfProceedRecord(userId);
	}
...
//	}
	...
	public Object queryRecordsByUserId(
		...
		Page<GroupProceedRecordPO> records = groupProceedRecordDao.findByUserIdOrderByStartTimeDesc(userId, page);
		long total = records.getTotalElements();
		List<GroupProceedRecordVO> vos = new ArrayList<GroupProceedRecordVO>();
		for(GroupProceedRecordPO record : records.getContent()){
			vos.add(new GroupProceedRecordVO().set(record));
