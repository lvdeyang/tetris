...
public class TreeNodeVO {
	...
	/**BusinessGroupMemberPO的id */
	/**FolderPO的id */
	private String id;
	
	private String uuid;
	
	/**BusinessGroupMemberPO的name */
	/**FolderPO的name */
	private String name;
	...
	/** jsonString 附加参数 */
	/**BusinessGroupMemberPO */
	/**FolderPO对应{"parentPath":"/376","parentId":376}*/
	...
	public TreeNodeVO setParentId(Long parentId) {
	 ...
	 * <b>作者:</b>lx<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2020年11月25日 下午4:29:44
	 * @param folder 文件夹
	 * @return TreeNodeVO 树节点
	 */
	public TreeNodeVO set(FolderPO folder){
		this.setId(folder.getId().toString())
			.setName(folder.getName())
			.setParam(JSON.toJSONString(new HashMapWrapper<String, Object>().put("parentId", folder.getParentId())
														  .put("parentPath", folder.getParentPath())
...
//		return this;
	 ...
	 * <b>作者:</b>lx<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2020年11月25日 下午4:53:38
	 * @param member 业务成员
	 * @param parentId 父节点
	 * @return TreeNodeVO 树节点
	 */
	public TreeNodeVO set(
			BusinessGroupMemberPO member,
			Long parentId){
		this.setId(member.getId().toString())
			.setUuid(member.getUuid())
			.setName(member.getName())
			.setParam(JSON.toJSONString(new HashMapWrapper<String, Object>().put("id", member.getId())
																			.put("name", member.getName())
																			.put("groupMemberType", member.getGroupMemberType().toString())
																			.put("groupMemberStatus", member.getGroupMemberStatus().toString())
																			.put("onlineStatus", member.getOnlineStatus().toString())
																			.put("roleId", member.getRoleId())
																			.put("roleName", member.getRoleName())
																		    .getMap()))
			.setType(TreeNodeType.MEMBE)
			.setIcon(TreeNodeIcon.SPOKESMAN.getName())
			.setParentId(parentId)
			.setKey(new StringBufferWrapper().append(this.generateKey()).append("@@").append(member.getOriginId()).toString());
	...
	public TreeNodeVO set(
			BusinessGroupMemberPO member,
			Long parentId,
			List<BusinessGroupMemberTerminalChannelPO> channelList){
		set(member, parentId);
		List<TreeNodeVO> channelTreeNodeList= channelList.stream().map(channel->{
			return new TreeNodeVO().set(channel);
		}).collect(Collectors.toList());
		this.setChildren(new ArrayListWrapper<TreeNodeVO>().addAll(channelTreeNodeList).getList());
		return this;
	}
...
//			.setKey(new StringBufferWrapper().append(this.generateKey()).append("@@").append(user.getCreater()).toString());
	...
	public TreeNodeVO setWithInfo(BusinessGroupMemberPO member, UserBO userBO, FolderPO folder){
		
		StringBufferWrapper name = new StringBufferWrapper().append(member.getName());
		if(member.getIsAdministrator()) name.append(" [主席]");
		if(folder != null) name.append(" [").append(folder.getName()).append("]");
		
		this.setId(member.getOriginId().toString())
			.setUuid(member.getUuid())
			.setName(name.toString())
			.setParam(JSON.toJSONString(new HashMapWrapper<String, Object>().put("id", Long.parseLong(member.getOriginId()))
																			.put("username", member.getName())
																			.put("status", "")
																		    .getMap()))
			.setType(GroupMemberType.MEMBER_USER.equals(member.getGroupMemberType())?TreeNodeType.USER:GroupMemberType.MEMBER_HALL.equals(member.getGroupMemberType())?TreeNodeType.CONFERENCE_HALL:null)
...
//			.setKey(new StringBufferWrapper().append(this.generateKey()).append("@@").append(user.getCreater()).toString());
	 ...
	 * <b>作者:</b>lx<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2020年11月25日 下午5:25:56
	 * @param channel
	 * @param bundle
	 * @return
	 */
	public TreeNodeVO set(BusinessGroupMemberTerminalChannelPO channel){
		
		
		this.setId(channel.getId().toString())
			.setName(channel.getTerminalChannelname())
			.setParam(JSON.toJSONString(new HashMapWrapper<String, Object>().put("id", channel.getId())
																			.put("name", channel.getTerminalChannelname())
																			.put("channelType", channel.getTerminalChannelType())
																			.getMap()))
			.setType(TreeNodeType.CHANNEL)
			.setIcon(TreeNodeIcon.CHANNEL.getName())
			.setKey(this.generateKey());
...
//	}
	...
	public TreeNodeVO set(GroupPO command, boolean entered){
		this.setId(command.getId().toString())
			.setUuid(command.getUuid())
			.setName(command.getName())
			.setType(TreeNodeType.COMMAND)
