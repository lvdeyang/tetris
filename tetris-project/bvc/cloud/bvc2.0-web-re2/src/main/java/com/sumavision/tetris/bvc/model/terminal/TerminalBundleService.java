...
public class TerminalBundleService {
	...
	public List<TerminalBundleVO> add(
		...
		Map<String, Object> entities = add(terminalId, name, bundleType, type, number, false);
		List<TerminalBundlePO> terminalBundles = (List<TerminalBundlePO>)entities.get("terminalBundles");
		return TerminalBundleVO.getConverter(TerminalBundleVO.class).convert(terminalBundles, TerminalBundleVO.class);
	}
	...
	public Map<String, Object> add(
			...
			String type, 
			Integer number,
			Boolean createTerminalChannel) throws Exception{
		...
		List<TerminalChannelPO> terminalChannels = new ArrayList<TerminalChannelPO>();
		Map<String, TerminalChannelPO> channelMap = new HashMap<String, TerminalChannelPO>();
		for(TerminalBundlePO bundle:bundles){
			for(String channelId:channelIds){
				...
				if(number==1 && createTerminalChannel){
					TerminalChannelPO terminalChannel = new TerminalChannelPO();
					terminalChannel.setName(new StringBufferWrapper().append(channel.getType().getName()).append(channel.getChannelId().split("_")[1]).toString());
					terminalChannel.setType(TerminalChannelType.valueOf(channel.getType().toString()));
					terminalChannel.setTerminalId(terminalId);
					terminalChannel.setUpdateTime(new Date());
					terminalChannels.add(terminalChannel);
					channelMap.put(channel.getChannelId(), terminalChannel);
				}
			}
		}
		if(channels.size() > 0) terminalBundleChannelDao.save(channels);
		if(terminalChannels.size() > 0) terminalChannelDao.save(terminalChannels);
		if(channelMap.size() > 0){
			List<TerminalChannelBundleChannelPermissionPO> permissions = new ArrayList<TerminalChannelBundleChannelPermissionPO>();
			...
			for(String channelId:keys){
				...
				TerminalChannelPO terminalChannel = channelMap.get(channelId);
				TerminalChannelBundleChannelPermissionPO permission = new TerminalChannelBundleChannelPermissionPO();
				permission.setTerminalBundleId(targetChannel.getTerminalBundleId());
				permission.setTerminalBundleChannelId(targetChannel.getId());
				permission.setTerminalChannelId(terminalChannel.getId());
				permission.setTerminalId(terminalChannel.getTerminalId());
				if(TerminalChannelType.VIDEO_DECODE.equals(terminalChannel.getType()) || 
						TerminalChannelType.VIDEO_ENCODE.equals(terminalChannel.getType())){
					permission.setChannelParamsType(ChannelParamsType.HD);
				}else{
					permission.setChannelParamsType(ChannelParamsType.DEFAULT);
				}
				permissions.add(permission);
			}
			terminalChannelBundleChannelPermissionDao.save(permissions);
		}
		
		Map<String, Object> entities = new HashMap<String, Object>();
		entities.put("terminalBundles", bundles);
		entities.put("terminalBundleChannels", channels);
		entities.put("terminalChannels", terminalChannels);
