...
public class LogicBO {
	...
	private List<RecordSetBO> recordSet = new ArrayList<RecordSetBO>();
	
	private List<RecordSetBO> recordUpdate = new ArrayList<RecordSetBO>();
	
	private List<RecordSetBO> recordDel = new ArrayList<RecordSetBO>();
	...
	private List<PublishStreamSetBO> publishStreamUpdate;
	
	private List<PublishStreamSetBO> publishStreamDel;
	...
	public List<PublishStreamSetBO> getPublishStreamDel() {
		return publishStreamDel;
	}

	public LogicBO setPublishStreamDel(List<PublishStreamSetBO> publishStreamDel) {
		this.publishStreamDel = publishStreamDel;
		return this;
	}
	...
	public List<PublishStreamSetBO> getPublishStreamUpdate() {
		return publishStreamUpdate;
	}

	public LogicBO setPublishStreamUpdate(List<PublishStreamSetBO> publishStreamUpdate) {
		this.publishStreamUpdate = publishStreamUpdate;
		return this;
	}
	...
	public LogicBO setCombineAudioSet_recon(Collection<BusinessCombineAudioPO> combineAudios, CodecParamBO codec){
		if(combineAudios!=null && combineAudios.size()>0){
			if(this.getCombineAudioSet() == null) this.setCombineAudioSet(new ArrayList<CombineAudioBO>());
			for(BusinessCombineAudioPO combineAudio:combineAudios){
				if(!combineAudio.isMix()) continue;
				CombineAudioBO protocalCombineAudio = new CombineAudioBO().set(combineAudio, codec);
				this.getCombineAudioSet().add(protocalCombineAudio);
			}
		}
	...
	public LogicBO setCombineAudioUpdate_recon(Collection<BusinessCombineAudioPO> combineAudios, CodecParamBO codec){
		if(combineAudios!=null && combineAudios.size()>0){
			if(this.getCombineAudioUpdate() == null) this.setCombineAudioUpdate(new ArrayList<CombineAudioBO>());
			for(BusinessCombineAudioPO combineAudio:combineAudios){
				CombineAudioBO protocalCombineAudio = new CombineAudioBO().set(combineAudio, codec);
	...
	public LogicBO setCombineAudioDel_recon(Collection<BusinessCombineAudioPO> combineAudios){
		if(combineAudios!=null && combineAudios.size()>0){
			if(this.getCombineAudioDel() == null) this.setCombineAudioDel(new ArrayList<CombineAudioBO>());
			for(BusinessCombineAudioPO combineAudio:combineAudios){
				CombineAudioBO protocalCombineAudio = new CombineAudioBO().setUuid(combineAudio.getUuid());
	 ...
	 * @return LogicBO 
	 */
	public LogicBO setCombineVideoSet_recon(Collection<BusinessCombineVideoPO> businessCombineVideos, CodecParamBO codec){
		if(businessCombineVideos!=null && businessCombineVideos.size()>0){
			if(this.getCombineVideoSet() == null) this.setCombineVideoSet(new ArrayList<CombineVideoBO>());
			for(BusinessCombineVideoPO businessCombineVideo:businessCombineVideos){
//				if(businessCombineVideo.isEffective()){
					CombineVideoBO protocalCombineVideo = new CombineVideoBO().set(businessCombineVideo, codec);
					this.getCombineVideoSet().add(protocalCombineVideo);
//				}
			}
		}
	...
	public LogicBO setCombineVideoUpdate_recon(Collection<BusinessCombineVideoPO> businessCombineVideos, CodecParamBO codec){
		if(businessCombineVideos!=null && businessCombineVideos.size()>0){
			if(this.getCombineVideoUpdate() == null) this.setCombineVideoUpdate(new ArrayList<CombineVideoBO>());
			for(BusinessCombineVideoPO businessCombineVideo:businessCombineVideos){
//				if(businessCombineVideo.isEffective()){
...
//				}
	...
	public LogicBO setCombineVideoDel_recon(Collection<BusinessCombineVideoPO> combineVideos){
		if(combineVideos!=null && combineVideos.size()>0){
			if(this.getCombineVideoDel() == null) this.setCombineVideoDel(new ArrayList<CombineVideoBO>());
	 ...
	 * @return LogicBO 协议数据
	 */
	public LogicBO setRecordDelByGroupRecord(Collection<GroupRecordPO> records, String transferToVod, GroupPO group){
		if(records!=null && records.size()>0){
			if(this.getRecordDel() == null) this.setRecordDel(new ArrayList<RecordSetBO>());
	 ...
	 * @return LogicBO 协议数据
	 */
	public LogicBO setGroupPublishStreamSet(Collection<GroupPublishStreamPO> publishStreams, CodecParamBO codec){
		if(publishStreams!=null && publishStreams.size()>0){
			if(this.getPublishStreamSet() == null) this.setPublishStreamSet(new ArrayList<PublishStreamSetBO>());
			for(GroupPublishStreamPO publishStream : publishStreams){
				PublishStreamSetBO protocalPublishStream = new PublishStreamSetBO().set(publishStream, codec);
				this.getPublishStreamSet().add(protocalPublishStream);
			}
		}
