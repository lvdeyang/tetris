...
//		task.setEndTime(parsedEndTime); //不需要结束时间
	 ...
	 */
	public void doScheduling(Map<String, List<MonitorRecordPO>> needDisposeRecords) throws Exception {

		Map<String, LogicBO> logics = new HashMap<String, LogicBO>();
		List<MonitorRecordPO> reallyNeedStartRecords = needDisposeRecords.get("start");
		
		if (reallyNeedStartRecords != null && reallyNeedStartRecords.size() > 0) {
			Set<Long> avtplIds = new HashSet<Long>();
			Set<Long> gearIds = new HashSet<Long>();
			for (MonitorRecordPO record : reallyNeedStartRecords) {
			...
			monitorRecordDao.save(reallyNeedStartRecords);
		}
...
//		
		List<MonitorRecordPO> reallyNeedStopRecords = needDisposeRecords.get("stop");
//		for(MonitorRecordPO record : needStopRecords){
//			if(record.getVersion() !=null && record.getVersion() == 0){
//				record.setVersion(1);
//				reallyNeedStopRecords.add(record);
//			}
//		}
		
		if (reallyNeedStopRecords != null && reallyNeedStopRecords.size() > 0) {
			for (MonitorRecordPO record : reallyNeedStopRecords) {
				...
				for(MonitorRecordPO stopRecord : reallyNeedStopRecords){
					stopRecord.setVersion(0);
				}
				monitorRecordDao.save(reallyNeedStopRecords);
			}
		}
...
*/
	...
	/**
	 * 坐标记修改version说明该录制已经有客户端处理<br/>
	 * <b>作者:</b>lx<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2021年4月2日 上午10:12:54
	 */
	public Map<String ,List<MonitorRecordPO>> saveVersion() {
		
		Map<String ,List<MonitorRecordPO>> needDisposeRecords = new HashMap<String, List<MonitorRecordPO>>();
		Date now = new Date();
		...
		List<MonitorRecordPO> reallyNeedStartRecords = new ArrayList<MonitorRecordPO>();
		for(MonitorRecordPO record : needStartRecords){
			if(record.getVersion() !=null && record.getVersion() == 0){
				record.setVersion(1);
				reallyNeedStartRecords.add(record);
			}
		}
		if(reallyNeedStartRecords.size() > 0){
			for(MonitorRecordPO record : reallyNeedStartRecords){
				monitorRecordDao.saveAndFlush(record);
			}
			needDisposeRecords.put("start", reallyNeedStartRecords);
		}
		...
		List<MonitorRecordPO> reallyNeedStopRecords = new ArrayList<MonitorRecordPO>();
		for(MonitorRecordPO record : needStopRecords){
			if(record.getVersion() !=null && record.getVersion() == 0){
				record.setVersion(1);
				reallyNeedStopRecords.add(record);
		...
		if(reallyNeedStopRecords.size() > 0){
			for(MonitorRecordPO record : reallyNeedStopRecords){
				monitorRecordDao.saveAndFlush(record);
			}
			needDisposeRecords.put("stop", reallyNeedStopRecords);
		}
		
		return needDisposeRecords;
	}
