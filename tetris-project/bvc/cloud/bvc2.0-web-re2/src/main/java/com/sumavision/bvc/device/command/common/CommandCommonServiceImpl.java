...
public class CommandCommonServiceImpl {
	...
	public Map<String, Object> queryDefaultAvCodec() throws Exception{
		return queryDefaultAvCodec(AvtplUsageType.COMMAND, GearsLevel.LEVEL_3);
	}
	public Map<String, Object> queryDefaultAvCodec(AvtplUsageType type, GearsLevel level) throws Exception{
		
		if(type == null) type = AvtplUsageType.COMMAND;
		if(level == null) level = GearsLevel.LEVEL_3;
		
		AvtplPO targetAvtpl = null;
		AvtplGearsPO targetGear = null;
		
		//查询codec参数
		List<AvtplPO> avTpls = avtplDao.findByUsageType(type);
		if(avTpls==null || avTpls.size()<=0){
			avtplService.generateDefaultAvtpls();
			avTpls = avtplDao.findByUsageType(type);
...
//			throw new AvtplNotFoundException("缺少会议.系统参数模板！");
		...
		//查询codec模板档位，优先取输入的档位，没有则get(0)取第一个
		List<AvtplGearsPO> gears = targetAvtpl.getGears();
		for(AvtplGearsPO gear:gears){
			if(targetGear == null) targetGear = gear;
			if(level.equals(gear.getLevel())) targetGear = gear;
		}
	...
	public CodecParamBO queryDefaultAvCodecParamBO() throws Exception{
		return queryDefaultAvCodecParamBO(AvtplUsageType.COMMAND, GearsLevel.LEVEL_3);
	}
	public CodecParamBO queryDefaultAvCodecParamBO(AvtplUsageType type, GearsLevel level) throws Exception{
		
		Map<String, Object> result = queryDefaultAvCodec(type, level);
...
//				}
	...
	/*TODO:public boolean whetherVideoCanBeDone(PageTaskPO task){
		if(commandBasicServiceImpl.whetherStopForCommandPause(forward)){
			return false;
		}else if(commandSecretServiceImpl.whetherStopForSecret(forward)){
			return false;
