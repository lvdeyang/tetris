			...
			if("start".equals(operate)){
				if(dstUser != null){
					if("play".equals(type)){
						...
						vodService.userStart(srcUser,dstUser,null,uuid);
					}else if("call".equals(type)){
						
						//开始xt用户呼叫本地用户，根据登录的不同平台，调用不同的方法
						if(dstUser.isLogined()){
							//这个登录状态是qt客户端的
							userCallService.userCallUser(srcUser, dstUser, -1, uuid, null);
						}else{
							//检查网页是否登录
...
//						commandUserServiceImpl.userCallUser_Cascade(srcUser, dstUser, -1, uuid);
				...
				if(dstUser != null){
					if("play".equals(type)){
					    ...
					    //根据uuid查询group
						VodPO vodPO=vodDAO.findByUuid(uuid);
						vodService.userStop(vodPO.getGroupId());
					}else if("call".equals(type)){
...
//						commandUserServiceImpl.stopCall_Cascade(UserClassify.LDAP.toString().equals(srcUser.getCreater())?srcUser:dstUser, null, uuid);
						userCallService.stopCall(UserClassify.LDAP.toString().equalsIgnoreCase(srcUser.getCreater())?srcUser:dstUser, null, uuid);
...
//						commandUserServiceImpl.acceptCall_Cascade(dstUser, null, uuid);
						userCallService.acceptCall(dstUser, null, uuid);
					}else if("play-call".equals(type)){
						//TODO: xt用户同意点播转呼叫
//						commandUserServiceImpl.acceptCall_Cascade(dstUser, null, uuid);
//						userCallService.acceptCall(dstUser, null, uuid);
