...
public class GroupSpeakService {
	...
	private Object speakAppoint(Long groupId, List<Long> memberIds) throws Exception{
		...
		//只能有1个发言人
		/*List<BusinessGroupMemberPO> speakingMembers = querySpeakMembers(groupId);
		...
		List<BusinessGroupMemberPO> speakMembers = new ArrayList<BusinessGroupMemberPO>();//统计本次新增的发言人
...
//				}
		...
		//给这些人授予发言人角色
		groupMemberRoleService.exchangeMemberRole(groupId,speakMembers.stream().map(BusinessGroupMemberPO::getId).collect(Collectors.toList()),speakRole.getId());
...
//		commandGroupDao.save(group);
		...
		if(!OriginType.OUTER.equals(group.getOriginType())){
			if(BusinessType.MEETING_BVC.equals(group.getBusinessType())){
				//级联指定发言只有单个协议，没有批量
				for(BusinessGroupMemberPO speakMember : speakMembers){
					GroupBO groupBO = commandCascadeUtil.speakerAgendaSet(group, speakMember);
	...
	public void speakApply(Long userId, Long groupId) throws Exception{
		...
		synchronized (new StringBuffer().append(lockProcessPrefix).append(groupId).toString().intern()) {
			...
			List<BusinessGroupMemberPO> members = businessGroupMemberDAO.findByGroupId(groupId);
			BusinessGroupMemberPO chairmanMember = tetrisBvcQueryUtil.queryChairmanMember(members);
			BusinessGroupMemberPO speakMember = tetrisBvcQueryUtil.queryBusinessMemberByUserId(members, userId);
...
//			}
			...
			if(!OriginType.OUTER.equals(chairmanMember.getOriginType())){
				...
				GroupBO groupBO = commandCascadeUtil.speakerAgendaSetRequest(group, speakMember);
				...
				if(memberIds.contains(member.getId())){
					...
					if(p == null){
						...
						if(!OriginType.OUTER.equals(chairmanMember.getOriginType())){
							...
							if(OriginType.OUTER.equals(member.getOriginType())){
								GroupBO groupBO = commandCascadeUtil.speakerAgendaSetResponse(group, member, "1");
								conferenceCascadeService.speakerSetResponse(groupBO);
							}

							// “成员主动申请发言通知”给所有其它系统
							GroupBO groupBO = commandCascadeUtil.speakerAgendaSet(group, member);
...
//			speakStart(group, speakMembers, 0);
	...
	@Transactional(rollbackFor = Exception.class)
	public void speakApplyDisagreeU(Long userId, Long groupId, List<Long> userIds) throws Exception{
		synchronized (new StringBuffer().append(lockProcessPrefix).append(groupId).toString().intern()) {
			List<Long> memberIds = businessCommonService.fromUserIdsToMemberIds(groupId, userIds);
			speakApplyDisagree(userId,groupId,memberIds);
		}
	}

	@Transactional(rollbackFor = Exception.class)
	public void speakApplyDisagreeM(Long userId, Long groupId, List<Long> memberIds) throws Exception{
		synchronized (new StringBuffer().append(lockProcessPrefix).append(groupId).toString().intern()) {
			speakApplyDisagree(userId,groupId,memberIds);
		}
	}
	...
	public void speakApplyDisagree(Long userId, Long groupId, List<Long> memberIds) throws Exception{
		...
		synchronized (new StringBuffer().append(lockProcessPrefix).append(groupId).toString().intern()) {
			...
			List<BusinessGroupMemberPO> members = businessGroupMemberDAO.findByGroupId(groupId);
			BusinessGroupMemberPO chairmanMember = tetrisBvcQueryUtil.queryChairmanMember(members);
			List<BusinessGroupMemberPO> speakMembers = tetrisBvcQueryUtil.queryMembersByIds(members, memberIds);
			...
			for(BusinessGroupMemberPO speakMember : speakMembers){
				...
				if(!OriginType.OUTER.equals(chairmanMember.getOriginType())){
					...
					if(OriginType.OUTER.equals(speakMember.getOriginType())){
						GroupBO groupBO = commandCascadeUtil.speakerAgendaSetResponse(group, speakMember, "0");
						conferenceCascadeService.speakerSetResponse(groupBO);
					}
				}
			}
...
//			}
		...
		RolePO speakRole = roleDao.findByInternalRoleType(InternalRoleType.MEETING_SPEAKER);
		List<BusinessGroupMemberPO> revokeMembers =new ArrayList<>();//统计本次解除的发言人
		List<GroupMemberRolePermissionPO> ps = groupMemberRolePermissionDao.findByRoleIdAndGroupMemberIdIn(speakRole.getId(), memberIds);
...
//		}
		...
		businessCommonService.notifyMemberInfo(notifyMemberIds, message.toJSONString(), WebsocketMessageType.COMMAND);
		RolePO audienceRole = roleDao.findByInternalRoleType(InternalRoleType.MEETING_AUDIENCE);
		groupMemberRoleService.exchangeMemberRole(groupId,revokeMembers.stream().map(BusinessGroupMemberPO::getId).collect(Collectors.toList()),audienceRole.getId());
        ...
        */
		...
		if(!OriginType.OUTER.equals(group.getOriginType())){
			if(BusinessType.MEETING_BVC.equals(group.getBusinessType())){
				for (BusinessGroupMemberPO speakMember:revokeMembers) {
					GroupBO groupBO = commandCascadeUtil.speakerAgendaSetCancel(user.getUserno(), group, speakMember);
					conferenceCascadeService.speakerSetCancel(groupBO);
				}
			}
		}
	...
	public void discussStart(Long userId, Long groupId) throws Exception{
		...
		synchronized (new StringBuffer().append(lockProcessPrefix).append(groupId).toString().intern()) {
			...
			BusinessType groupType = group.getBusinessType();
			if(!groupType.equals(BusinessType.MEETING_BVC)){
			...
			agendaExecuteService.runAgenda(groupId,discussAgenda.getId(),businessDeliver,true);
			//级联
			if(!OriginType.OUTER.equals(group.getOriginType())){
				GroupBO groupBO = commandCascadeUtil.discussAgendaStart(group);
...
//				throw new BaseException(StatusCode.FORBIDDEN, group.getName() + " 会议已停止，无法操作，id: " + group.getId());
            ...
            businessCommonService.notifyMemberInfo(notifyMemberIds, message.toJSONString(), WebsocketMessageType.COMMAND);
			//停止讨论议程，执行会议议程
			AgendaPO basicAgenda = agendaDao.findByBusinessInfoType(BusinessInfoType.BASIC_MEETING);
			agendaExecuteService.runAgenda(groupId,basicAgenda.getId(),businessDeliver,true);



			//级联
			if(!OriginType.OUTER.equals(group.getOriginType())){
				GroupBO groupBO = commandCascadeUtil.discussAgendaStop(group);
	...
	private boolean isGroupUnderDiscussion(Long groupId){
		AgendaPO discussAgenda = agendaDao.findByBusinessInfoType(BusinessInfoType.MEETING_DISCUSS);
		if(discussAgenda == null) return false;
	...
	private List<BusinessGroupMemberPO> querySpeakMembers(Long groupId){
		RolePO speakRole = roleDao.findByInternalRoleType(InternalRoleType.MEETING_SPEAKER);
		List<BusinessGroupMemberPO> speakMembers = businessGroupMemberDAO.findByGroupIdAndRoleId(groupId, speakRole.getId());
		return speakMembers;
	}
}
