...
public class PageTaskService {
	...
	public MessageSendCacheBO notifyUser(CommandGroupUserInfoPO userInfo, PageInfoPO pageInfo, boolean doWebsocket) throws Exception{
		...
		CommandGroupUserLayoutShemeVO schemeVO = generateSchemeVO(pageInfo);
...
//		log.info("推送" + userInfo.getUserName() + "用户的当前分页layout: " + JSON.toJSONString(schemeVO));
		...
		MessageSendCacheBO cache = new MessageSendCacheBO(Long.parseLong(pageInfo.getOriginId()), message.toJSONString(), WebsocketMessageType.COMMAND);
	
		//发送消息
		if(doWebsocket){
			if(businessReturnService.getSegmentedExecute()){
				businessReturnService.add(null, cache, "推送id: " + pageInfo.getOriginId() + " 用户的当前分页layout: " + JSON.toJSONString(schemeVO));
			}else{
				WebsocketMessageVO ws = websocketMessageService.send(Long.parseLong(pageInfo.getOriginId()), message.toJSONString(), WebsocketMessageType.COMMAND);
	...
	public void addAndRemoveTasks(PageInfoPO pageInfo, List<PageTaskPO> newTasks, List<PageTaskPO> removeTasks) throws Exception{
		List<PageTaskPO> s = pageInfo.getPageTasks();
...
//			if(businessId.matches("\\d+")){
				...
				if(group!=null&&group.getLocationIndex()!=null&&group.getLocationIndex()!=-1){
					if(BusinessType.CALL.equals(group.getBusinessType()) && group.getLocationIndex() != null && group.getLocationIndex() != -1 ){
						
						RolePO callerRolePO = roleDao.findByInternalRoleType(InternalRoleType.CALLER_USER);
						if(callerRolePO == null){
							callerRolePO = new RolePO();
						}
						for(BusinessGroupMemberPO member : group.getMembers()){
							for(PageTaskPO task : newTasks){
								if(member.getRoleId().equals(callerRolePO.getId()) && member.getId().equals(task.getDstMemberId())){
									task.setTaskIndex(group.getLocationIndex());
									task.setFixedAtPageAndLocation(true);
								}
							}
						}
//						&& member.getRoleId().equals(callerRolePO.getId())
//						task.setLocationIndex(group.getLocationIndex());
					}else{
...
//			}
		...
		for(PageTaskPO removeTask : removeTasks){
			if(removeTask.getCastDevices()!=null && removeTask.getCastDevices().size()>0){
...
//		tasks.removeAll(removeTasks);
	...
	private LogicBO comparePage(
		...
		synchronized (pageInfo.getOriginId()) {//TODO:这个锁需要再考虑
			...
			//对将要显示的设置参数
			int index=0;
			if(newPageTasks == null) newPageTasks = new ArrayList<PageTaskPO>();
...
//				newPageTask.setLocationIndex(newTaskIndex);
				...
				for(PageTaskPO oldPageTask : oldPageTasks){
					if(newPageTask.isTheSameTask(oldPageTask)){//对比2个task是否相同（当前仅以id为标准）
						has = true;
...
//			oldPageTasks.removeAll(newPageTasks);
			...
			if(GroupMemberType.MEMBER_HALL.equals(pageInfo.getGroupMemberType())
					|| GroupMemberType.MEMBER_DEVICE.equals(pageInfo.getGroupMemberType())){
...
//						LayoutPositionPO position = layoutPositionDao.findOne(addTask.getPositionId());
						...
						Boolean enablePosition = addTask.getEnablePosition();
						if(Boolean.TRUE.equals(enablePosition)){
//							LayoutForwardPO layoutForward = layoutForwardDao
						}
						/*if(positionId != null){
						...
						//给未指定通道的task设置目的。给指定通道的设音频解码通道（暂时写死）
						if(addTask.getTerminalVideoDecodeChannelId() == null || addTask.getTerminalVideoDecodeChannelId() == -1L){
							...
							addTask.setDstAudioBaseType("VenusAudioOut");
							addTask.setDstAudioChannelId(ChannelType.AUDIODECODE1.getChannelId());
							addTask.setDstAudioChannelName(ChannelType.AUDIODECODE1.getName());
						}
				...
				GroupPO group =null;
				for (PageTaskPO pageTask:addTasks) {
					group=groupDao.findOne(pageTask.obtainGroupId());
					if(group != null)break;
				}
				...
				logic = openAndCloseDecoder_Rect(addTasks, closeTasks, codec);//这里传入closeTasks或者oldPageTasks，可以再考虑
	...
	//				newTaskPlayers.addAll(players);
				...
				//呼叫所有的newTaskPlayers，挂断所有的NONE的needClosePlayers；进行相应的上屏处理；字幕下发
//				CodecParamBO codec = commandCommonServiceImpl.queryDefaultAvCodecParamBO(AvtplUsageType.COMMAND, GearsLevel.LEVEL_3);
				CodecParamBO codec = null;
				GroupPO group =null;
				for (PageTaskPO pageTask:addTasks) {
					group=groupDao.findOne(pageTask.obtainGroupId());
				...
				if(group == null){
					codec = commandCommonServiceImpl.queryDefaultAvCodecParamBO(AvtplUsageType.DEVICE_GROUP, GearsLevel.LEVEL_3);
				}else{
					DeviceGroupAvtplGearsPO gear = multiRateUtil.queryDefaultGear(group.getAvtpl().getGears());
					codec = new CodecParamBO().set(gear);
				}
	...
	public LogicBO reExecute(
		...
		CodecParamBO codec = null;
		GroupPO group =null;
		for (PageTaskPO pageTask:showingPageTasks) {
			group=groupDao.findOne(pageTask.obtainGroupId());
		}
	...
	public CommandGroupUserLayoutShemeVO generateSchemeVO(PageInfoPO pageInfo) throws BaseException{
		List<PageTaskPO> pageTasks = getPageTasks(pageInfo, pageInfo.getCurrentPage(), true);
		int totalPageNumber = getTotalPageNumber(pageInfo);
		CommandGroupUserLayoutShemeVO schemeVO = generateSchemeVO(pageInfo, pageTasks, totalPageNumber);
		return schemeVO;
	}

	private CommandGroupUserLayoutShemeVO generateSchemeVO(
			PageInfoPO pageInfo, List<PageTaskPO> pageTasks, int totalPageNumber){
		...
		if(pageInfo.getPlayerSplitLayout() == null){
			try {
				pageInfo.setPlayerSplitLayout(PlayerSplitLayout.fromPlayerCount(pageInfo.getPageSize()));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		vo.setPlayerSplitLayout(String.valueOf(pageInfo.getPlayerSplitLayout().getId()));
		vo.setPlayersByPageTasks(pageTasks);
		
		int allNumber = pageInfo.getPlayerSplitLayout().getPlayerCount();
...
//		}
	...
	private LogicBO openAndCloseDecoder(
		...
		for(PageTaskPO openTask : openTasks){
			...
			CodecParamBO thisCodec = codec;
			Long gearId = openTask.getGearId();
			if(gearId != null){
				DeviceGroupAvtplGearsPO gear = deviceGroupAvtplGearsDao.findOne(gearId);
				if(gear != null) thisCodec = new CodecParamBO().set(gear);
			}
...
//			  													        .setOperateType(ConnectBundleBO.OPERATE_TYPE)
			...
			if(SourceType.COMBINE_VIDEO.equals(openTask.getVideoSourceType())){
			...
			if(SourceType.COMBINE_AUDIO.equals(openTask.getAudioSourceType())){
...
//																           	 .setOperateType(DisconnectBundleBO.OPERATE_TYPE)
	 ...
	 * @param codec 传入一个默认codec，以便在task.getGearId()找不到DeviceGroupAvtplGearsPO的时候使用
	 * @return
	 */
	public LogicBO openAndCloseDecoder_Rect(
			List<PageTaskPO> openTasks,
			List<PageTaskPO> _closeTasks,
			CodecParamBO codec){
		
		if(openTasks == null) openTasks = new ArrayList<PageTaskPO>();
		if(_closeTasks == null) _closeTasks = new ArrayList<PageTaskPO>();
		List<PageTaskPO> closeTasks = new ArrayList<PageTaskPO>(_closeTasks);
		
		//去掉相同解码通道的close
		
		Map<String, List<PageTaskPO>> map = pageTaskUtil.classifyBundleTasks(openTasks);
		Map<String, List<PageTaskPO>> closeMap = pageTaskUtil.classifyBundleTasks(closeTasks);
...
//				connectDecoderBundle.setChannels(new ArrayListWrapper<ConnectBO>().add(connectDecoderVideoChannel).add(connectDecoderAudioChannel).getList());
				...
				List<String> channelIdList = connectDecoderBundle.getChannels().stream().map(ConnectBO::getChannelId).collect(Collectors.toList());
				if(!channelIdList.contains(connectDecoderVideoChannel.getChannelId())){
					connectDecoderBundle.getChannels().add(connectDecoderAudioChannel);
				}
				if(!channelIdList.contains(connectDecoderAudioChannel.getChannelId())){
					connectDecoderBundle.getChannels().add(connectDecoderAudioChannel);//TODO:虚拟源时可能产生重复的多个音频解码channel，需要去掉
				}
//				logic.getConnectBundle().add(connectDecoderBundle);

				//分屏显示（大小屏）
				Boolean enablePosition = openTask.getEnablePosition();
				if(enablePosition){
					Long layoutForwardId = openTask.getLayoutForwardId();
					LayoutForwardPO layoutForward = LayoutForwardDao.findOne(layoutForwardId);
					if(layoutForward != null){
						if(screen == null) screen = new ScreenBO().setScreen_id("screen_1").setRects(new ArrayList<RectBO>());
						RectBO rect = new RectBO().set(layoutForward, openTask.getDstVideoChannelId());
						screen.getRects().add(rect);
					}
				}
			}
			List<PageTaskPO> closes = closeMap.get(bundleId);
			if(closes !=null){
				for(PageTaskPO closeTask : closes){
					
					CodecParamBO thisCodec = codec;
					Long gearId = closeTask.getGearId();
					if(gearId != null){
						DeviceGroupAvtplGearsPO gear = deviceGroupAvtplGearsDao.findOne(gearId);
						if(gear != null) thisCodec = new CodecParamBO().set(gear);
					}
					...
					//视频
					ConnectBO connectDecoderVideoChannel = new ConnectBO().setChannelId(closeTask.getDstVideoChannelId())
					          .setChannel_status("Open")
					          .setBase_type(closeTask.getDstVideoBaseType())
					          .setMode(closeTask.getVideoTransmissionMode().getCode())
					          .setMulti_addr(closeTask.getVideoMultiAddr())
					          .setSrc_multi_ip(closeTask.getVideoMultiSrcAddr())
					          .setCodec_param(thisCodec);
					connectDecoderBundle.getChannels().add(connectDecoderVideoChannel);
					
					//是否有音频
					if(closeTask.getSrcAudioBundleId() != null && closeTask.getSrcAudioChannelId() != null){
						ConnectBO connectDecoderAudioChannel = new ConnectBO().setChannelId(closeTask.getDstAudioChannelId())
																			  .setChannel_status("Open")
																			  .setBase_type(closeTask.getDstAudioBaseType())
																	          .setMode(closeTask.getAudioTransmissionMode().getCode())
																	          .setMulti_addr(closeTask.getAudioMultiAddr())
																	          .setSrc_multi_ip(closeTask.getAudioMultiSrcAddr())
																			  .setCodec_param(thisCodec);
						connectDecoderBundle.getChannels().add(connectDecoderAudioChannel);
					}
				}
		...
		//处理剩下的closeMap：只有清除源，没有增加源的
		for(String bundleId : closeMap.keySet()){
			List<PageTaskPO> closes = closeMap.get(bundleId);
			PageTaskPO close = closes.get(0);
			ConnectBundleBO connectDecoderBundle = new ConnectBundleBO().setBusinessType(ConnectBundleBO.BUSINESS_TYPE_VOD)
//				        .setOperateType(ConnectBundleBO.OPERATE_TYPE)
				        .setLock_type("write")
				        .setBundleId(close.getDstBundleId())
				        .setLayerId(close.getDstLayerId())
				        .setBundle_type(close.getDstBundleType());
			for(PageTaskPO closeTask : closes){
...
//																           	 .setOperateType(DisconnectBundleBO.OPERATE_TYPE)
	...
	private PageTaskPO newAddTask(PageInfoPO pageInfo, List<PageTaskPO> newTasks ){
		...
		if(pageTasks==null){
			pageTasks=new ArrayList<PageTaskPO>();
			pageInfo.setPageTasks(pageTasks);
		} 
