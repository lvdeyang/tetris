...
//	}
	...
	public List<Long> fromUserIdsToMemberIds(Long groupId, List<Long> userIds) {
		...
		List<Long> memberIds = businessGroupMemberDAO.findIdsByGroupIdAndOriginIds(groupId, originIds);
		return memberIds;
	}

	/** 只能查出用户成员，设备成员会被忽略 */
	public List<UserBO> queryUsersByGroupMembers(Collection<BusinessGroupMemberPO> members) {
	...
	/*获取成员名字*/
	public List<String> obtainMemberNames(List<BusinessGroupMemberPO> members) {
	...
	public List<Long> obtainCooperateMemberIds(Long groupId,Long roleId,boolean connected) {
		List<Long> memberIds = new ArrayList<Long>();
		List<BusinessGroupMemberPO> members = businessGroupMemberDAO.findByGroupIdAndRoleId(groupId,roleId);
		for (BusinessGroupMemberPO member : members) {
			boolean fit = true;
			if (connected) {
				if (!GroupMemberStatus.CONNECT.equals(member.getGroupMemberStatus())) {
	...
	public List<Long> obtainMemberIds(List<BusinessGroupMemberPO> members) {
		List<Long> memberIds = new ArrayList<Long>();
		for (BusinessGroupMemberPO member : members) {
			memberIds.add(member.getId());
		}
	...
	public void notifyMemberInfo(List<Long> memberIds, String message, WebsocketMessageType type) throws Exception {
		...
		List<BusinessGroupMemberPO> members = businessGroupMemberDAO.findAll(memberIds);
