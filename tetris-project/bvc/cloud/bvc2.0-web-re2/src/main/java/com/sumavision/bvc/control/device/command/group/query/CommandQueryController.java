...
//					(!"ldap".equals(user.getCreater()) && userMap!=null && userMap.getEncodeBundleId()!=null) && !userMap.getEncodeBundleId().equals("")){//过滤掉编码器uuid为null和空字符串，其它情况无法过滤
		...
		if(roleIds.size() > 0){
			...
			conferenceHalls = conferenceHallDao.findAll(conferenceHallIds).stream().filter(hall -> {
				if(hall.getFromDevice() == null){
					return true;
				}
				return !hall.getFromDevice();
...
//		List<UserBO> users = resourceService.queryUserresByUserId(userId);
	...
	public Object findInstitutionTreeUserExceptCommand(String id, HttpServletRequest request) throws Exception{ 
		...
		List<BusinessGroupMemberPO> groupMembers = businessGroupMemberDao.findByGroupId(Long.parseLong(id));
		List<Long> existUserIds = new ArrayList<Long>();
		List<Long> existHallIds = new ArrayList<Long>();
		for(BusinessGroupMemberPO groupMember:groupMembers){
...
//			needType = GroupType.MEETING;
			needBusinessType = BusinessType.MEETING_BVC;
			break;
		case "command":
		default:
//			needType = GroupType.BASIC;
	 ...
	 * <b>作者:</b>lx<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2020年12月1日 下午5:41:12
	 * @param type 查询类型，command或meeting，如果null则认为command
	 * @return list 会议集合
	 */
	@JsonBody
	@ResponseBody
	@RequestMapping(value = "/create/by/myself/list")
	public Object creatByMyself(HttpServletRequest request) throws Exception{
...
//		BusinessType needBusinessType = null;
		...
		List<GroupPO> commands = new ArrayList<GroupPO>();
		if(user.getIsGroupCreator()){
			commands = groupDAO.findAll();
		}else{
			commands = groupDAO.findByUserId(user.getId());
		}
		
		//自己创建的会议列表
		List<GroupVO> myselfGroupList = new ArrayList<GroupVO>(); 
		
		for(GroupPO command: commands){
			myselfGroupList.add(new GroupVO().set(command));
		}
		
		return myselfGroupList;
	}
	...
	public Object findEnterdCommandGroup(
		...
		switch (type){
		case "meeting":
			needType = BusinessType.MEETING_BVC;
			break;
		case "command":
		default:
	...
	public Object findCommandForwardDemand(
		...
		List<AgendaForwardPO> agendaForwardList = agendaForwardDao.findByBusinessIdAndAgendaForwardBusinessType(Long.valueOf(id), BusinessInfoType.COMMAND_FORWARD);
		List<Long> forwardIdList = agendaForwardList.stream().map(AgendaForwardPO::getId).collect(Collectors.toList());
		Page<AgendaForwardDemandPO> demands = agendaForwardDemandDao.findByAgendaForwardIdInOrBusinessId(forwardIdList, Long.valueOf(id), page);
...
//	}
	...
	/**
	 * 查找GroupPO的groupLock的类型<br/>
	 * <b>作者:</b>lx<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2021年1月7日 下午4:35:57
	 * @return
	 */
	@JsonBody
	@ResponseBody
	@RequestMapping(value = "/query/group/lock")
	public Object queryGroupLock() throws Exception{
		GroupLock[] values = GroupLock.values();
		ArrayList<HashMap<String, String>> locks = new ArrayList<HashMap<String,String>>();
	...
	/**
	 * 查找GroupPO的BusinessOrderGroupType的类型<br/>
	 * <b>作者:</b>lx<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2021年1月7日 下午4:37:38
	 * @return
	 */
	@JsonBody
	@ResponseBody
	@RequestMapping(value = "/query/order/group/type")
	public Object queryOrderGroupType() throws Exception{
