...
public class MeetingEntityUtil {
	...
	/**
	 * 对于同一个设备的锁定，把不需要计数的connectBundle合并到1个需要计数的里边去。如果全都不计数，则不合并<br/>
	 * <p>由于资源层批量锁定同一个设备时，无法处理一部分计数、一部分不计数的情况，所以这里把它们合并成1个锁定</p>
	 * <b>作者:</b>zsy<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2021年1月14日 下午5:35:17
	 * @param connectBundle
	 * @return
	 */
	public static JSONArray combineNotCountToCount(JSONArray connectBundle){
		
		if(connectBundle == null || connectBundle.size()<2) return connectBundle;
		
		Map<String, JSONArray> bundleIdConnectBundlesMap = new HashMap<String, JSONArray>();
		for(int i=0; i<connectBundle.size(); i++){
			JSONObject aConnectBundle = connectBundle.getJSONObject(i);
			String bundleId = aConnectBundle.getString("bundleId");
			JSONArray bundles = bundleIdConnectBundlesMap.get(bundleId);
			if(bundles == null){
				bundles = new JSONArray();
				bundleIdConnectBundlesMap.put(bundleId, bundles);
			}
			bundles.add(aConnectBundle);
		}
		
		JSONArray result = new JSONArray();
		for(String bundleId : bundleIdConnectBundlesMap.keySet()){
			...
			//【找到1个需要计数的connectBundle】和所有不需要计数的，合并；其它需要计数的，直接放入result
			JSONObject countBundle = null;
			JSONArray notcountBundles = new JSONArray();
			for(int i=0; i<bundles.size(); i++){
				JSONObject bundle = bundles.getJSONObject(i);
				if(ConnectBundleBO.BUSINESS_TYPE_VOD.equals(bundle.getString("businessType"))
						&& ConnectBundleBO.OPERATE_TYPE.equals(bundle.getString("operateType"))){
					if(countBundle == null){
						countBundle = bundle;
					}else{
						result.add(bundle);
					...
					notcountBundles.add(bundle);
				}
			}
			if(countBundle != null){
				
				//合并通道和屏幕
				JSONArray countChannels = countBundle.getJSONArray("channels");
				if(countChannels == null) countChannels = new JSONArray();
				JSONArray countScreens = countBundle.getJSONArray("screens");
				if(countScreens == null) countScreens = new JSONArray();
				for(int i=0; i<notcountBundles.size(); i++){
					JSONObject notcountBundle = notcountBundles.getJSONObject(i);
					//通道
					JSONArray notcountChannels = notcountBundle.getJSONArray("channels");
					if(notcountChannels != null){
						countChannels.addAll(notcountChannels);
					}
					//屏幕
					JSONArray notcountScreens = notcountBundle.getJSONArray("screens");
					if(notcountScreens != null){
						countScreens.addAll(notcountScreens);
					}
					
				}
				
				//重新put一次
				countBundle.put("channels", countChannels);
				countBundle.put("screens", countScreens);				
				result.add(countBundle);
				
			}else{
				result.addAll(notcountBundles);
			}
		}
		return result;
	}
...
//				String connectLayerId = aConnect.getString("layerId");
	...
	/*
	 * 将connectBundle合并到disconnectBundle中，也就是disconnectBundle的不需要再connectBundle，以挂断为主
	 */
	public static void connectBundleIntoDisconnectBundle(JSONArray connectBundle, JSONArray disconnectBundle){
		//delIndex用于记录需要删除的index（不能在遍历途中直接删除）
		List<Integer> delIndex = new LinkedList<Integer>();
		for(int i=0; i<disconnectBundle.size(); i++){
			...
			for(int j=0; j<connectBundle.size(); j++){
				...
				String disconncetBundleId = aDisconnectBundle.getString("bundleId");
				String connectBundleId = aConnectBundle.getString("bundleId");
				if(disconncetBundleId.equals(connectBundleId)){
					delIndex.add(j);
					break;
				}
			}
		}
		//根据delIndex删除connect中的元素
		Collections.sort(delIndex);
		for(int j=delIndex.size()-1; j>=0; j--){
			connectBundle.remove(Integer.parseInt(delIndex.get(j).toString()));
		}
	}
