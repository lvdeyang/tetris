package com.sumavision.bvc.control.device.command.group.cross;
...
@Controller
@RequestMapping(value = "/command/cross")
public class CommandCrossController {
...
//	private CommandCooperateServiceImpl commandCooperateServiceImpl;
	...
	public Object start(
		...
		groupCrossService.startU(Long.parseLong(id), userIdArray);
...
//		JSONArray result = commandCooperateServiceImpl.revokeBatch(userIdArray, Long.parseLong(id));
		businessReturnService.init(Boolean.TRUE);
		groupCrossService.revokeBatchM(Long.parseLong(id),userIdArray);
	...
	public Object stop(
		...
		businessReturnService.init(Boolean.TRUE);
		groupCrossService.revokeBatchM(Long.parseLong(id), null);//第二个参数并没有用
