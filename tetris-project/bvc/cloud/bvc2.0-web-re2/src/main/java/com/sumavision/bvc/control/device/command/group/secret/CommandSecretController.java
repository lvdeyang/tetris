...
public class CommandSecretController {
	...
	@Autowired
	GroupSecretService groupSecretService;
	
	@Autowired
	BusinessReturnService businessReturnService;
	...
	public Object start(
		...
		groupSecretService.startSecret(Long.parseLong(id), user.getId(), user.getId(), Long.parseLong(userId));
//		groupSecretService.stopSecret(Long.parseLong(id), user.getId());//test
	...
	public Object startSpecifyPlayer(
		...
		throw new BaseException(StatusCode.FORBIDDEN, "请从成员列表中发起专向");
...
//		return chairSplits;
	...
	public Object stop(
		...
		groupSecretService.stopSecret(Long.parseLong(id), user.getId());
