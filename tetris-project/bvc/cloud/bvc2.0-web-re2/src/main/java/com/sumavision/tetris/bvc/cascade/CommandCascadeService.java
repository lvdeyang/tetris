...
public class CommandCascadeService {
	 ...
	 * <b>作者:</b>lx<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2021年2月24日 下午5:19:47
	 * @param String gid 组id
	 * @param String op 操作用户号码
	 * @param String upid 上级用户id
	 * @param String downid 下级用户id
	 */
	public void startSecret(GroupBO group) throws Exception{
		String fullName = generateFullName("secretCmdStart.xml");
		Template template = templateLoader.load(fullName);
		sendPassBy(group, NO_TYPE_APP, template);
	}
	 ...
	 * @param String downid 下级用户id
	 */
	public void stopSecret(GroupBO group) throws Exception{
		String fullName = generateFullName("secretCmdStop.xml");
		Template template = templateLoader.load(fullName);
		sendPassBy(group, NO_TYPE_APP, template);
	}
	 ...
	 * <b>日期：</b>2021年2月24日 下午5:19:47
	 * @param String gid 组id
	 * @param String op topid接替人
	 * @param String targid mid被接替人
	 */
	public void startReplace(GroupBO group) throws Exception{
		String fullName = generateFullName("replaceCmdStart.xml");
		Template template = templateLoader.load(fullName);
		sendPassBy(group, NO_TYPE_APP, template);
	}
	 ...
	 * @param String targid mid被接替人
	 */
	public void stopReplace(GroupBO group) throws Exception{
		String fullName = generateFullName("replaceCmdStop.xml");
		Template template = templateLoader.load(fullName);
		sendPassBy(group, NO_TYPE_APP, template);
	}
	 ...
	 * @param String gid 组id
	 * @param String op 操作人
	 * @param String accepauthid mid授权接收人
	 * @param String cmdedid topid被指挥人
	 */
	public void startAuthorize(GroupBO group) throws Exception{
		String fullName = generateFullName("authCmdStart.xml");
		Template template = templateLoader.load(fullName);
		sendPassBy(group, NO_TYPE_APP, template);
	}
	 ...
	 * @param String cmdedid topid被指挥人
	 */
	public void stopAuthorize(GroupBO group) throws Exception{
		String fullName = generateFullName("authCmdStop.xml");
		Template template = templateLoader.load(fullName);
		sendPassBy(group, NO_TYPE_APP, template);
	}
	 ...
	 * @param String gid 组id
	 * @param String op 操作人
	 * @param String upid topid 高级别成员
	 * @param String downid mid 低级别成员
	 */
	public void startCross(GroupBO group) throws Exception{
		String fullName = generateFullName("crossCmdStart.xml");
		Template template = templateLoader.load(fullName);
		sendPassBy(group, NO_TYPE_APP, template);
	}
	 ...
	 * @param String downid mid 低级别成员
	 */
	public void stopCross(GroupBO group) throws Exception{
		String fullName = generateFullName("crossCmdStop.xml");
		Template template = templateLoader.load(fullName);
		sendPassBy(group, NO_TYPE_APP, template);
	}
	 ...
	 * @param List<MinfoBO> mlist 需要级联的成员信息（全员）
	 */
	public void startTemp(GroupBO group) throws Exception{
		String fullName = generateFullName("tempStartCmd.xml");
		Template template = templateLoader.load(fullName);
		sendPassBy(group, NO_TYPE_APP, template);
	}
