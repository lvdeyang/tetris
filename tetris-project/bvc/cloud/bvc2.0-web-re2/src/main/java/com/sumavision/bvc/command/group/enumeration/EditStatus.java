...
public enum EditStatus {
	...
	NORMAL("正常", "preset"),//预设
	TEMP("临时", "temp");//保存后变为“正常”，如果不保存则停止后直接删除
//	DELETE("已删除");//待定，如果做了录制，那么删除需要通过指挥组来呈现录制文件
	
	private String name;
	
	private String code;//大白本协议字段
	
	private EditStatus(String name, String code){
		this.name = name;
		this.code = code;
	}
	...
	public static EditStatus fromCode(String code) throws Exception{
		EditStatus[] values = EditStatus.values();
		for(EditStatus value:values){
