...
public class AvtplGearsController {
	...
	public Object load(
		...
		List<AvtplGearsPO> gears = avtplDAO.findOne(id).getGears();
	...
	public Object save(
			...
			String name,
			String videoBitRate,
			String videoBitRateSpare,//（二轮重构多码率不再使用）
			String videoResolution,
			String videoResolutionSpare,//（二轮重构多码率不再使用）
			String fps,
			String videoFormat,
			String audioFormat,
			String audioBitRate,
		...
		if(tpl.getGears() == null) tpl.setGears(new ArrayList<AvtplGearsPO>());
		...
		gear.setFps(fps);
		gear.setVideoFormat(videoFormat==null?null:VideoFormat.fromName(videoFormat));
		gear.setAudioFormat(audioFormat==null?null:AudioFormat.fromName(audioFormat));
