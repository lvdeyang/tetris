...
public class TerminalBundleChannelQuery {
	...
	/**
	 * 根据终端和通道类型查询带设备信息的通道<br/>
	 * <b>作者:</b>lvdeyang<br/>
	 * <b>版本：</b>1.0<br/>
	 * <b>日期：</b>2020年6月19日 上午9:03:11
	 * @param Long terminalBundleId 终端设备id
	 * @param String type 通道类型
	 * @return List<TerminalBundleChannelVO> 带设备信息的通道列表
	 */
	public List<TerminalBundleChannelVO> loadAllByType(
			Long terminalId,
			String type) throws Exception{
		List<TerminalBundleChannelWithTerminalBundleAndParamsPermissionDTO> entities = terminalBundleChannelDao.findByTerminalIdAndType(terminalId, TerminalBundleChannelType.valueOf(type));
		List<TerminalBundleChannelVO> channels = new ArrayList<TerminalBundleChannelVO>();
		if(entities!=null && entities.size()>0){
			for(TerminalBundleChannelWithTerminalBundleAndParamsPermissionDTO entity:entities){
				channels.add(new TerminalBundleChannelVO().set(entity));
			}
		}
		return channels;
	}
