...
//					String connectChannelId = aDisconnectBundle.getString("channelId");
		...
		//由于资源层批量锁定同一个设备时，无法处理一部分计数、一部分不计数的情况，所以这里把它们合并成1个锁定（2021.1.14）
		JSONArray connectBundle = combinedOperation.getJSONArray("connectBundle");
		JSONArray combinedConnectBundle = MeetingEntityUtil.combineNotCountToCount(connectBundle);
		combinedOperation.put("connectBundle", combinedConnectBundle);
		
		//2021.3.10 将connectBundle合并到disconnectBundle中，也就是disconnectBundle的不需要再connectBundle，以挂断为主
		//在停会时，会有挂断编码器（disconnectBundle），和删除转发源（connectBundle）同时下发，此时去掉connectBundle以disconnectBundle为主
		//但是如果disconnectBundle没有把计数减为0，则会不下发任何命令
		if(combinedOperation.containsKey("connectBundle") && combinedOperation.containsKey("disconnectBundle")){
			JSONArray disconnectBundle = combinedOperation.getJSONArray("disconnectBundle");
			MeetingEntityUtil.connectBundleIntoDisconnectBundle(connectBundle, disconnectBundle);
		}		
		...
		for(int i=0; i<position.size(); i++){
			...
			if(aPosition.containsKey("operation") && aPosition.getString("operation").equals("next_mixer_video_loop_index")){
				...
				if(null != combineVideoDstPO){
					...
					screen.put("z_index", aPosition.getInteger("z_index"));
