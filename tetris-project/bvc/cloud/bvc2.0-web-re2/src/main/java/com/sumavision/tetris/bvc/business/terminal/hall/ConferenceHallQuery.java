...
public class ConferenceHallQuery {
	...
	public List<TerminalVO> loadSingleBundleTerminal() throws Exception{
		List<TerminalType> types = TerminalType.findBySignleBundle(true);
		List<TerminalPO> terminalEntities = terminalDao.findByTypeIn(types);
		return TerminalVO.getConverter(TerminalVO.class).convert(terminalEntities, TerminalVO.class);
	}
	...
	public Map<String, Object> load(
		...
		if(name==null || "".equals(name)){
			...
			pagedEntities = conferenceHallDao.findByNameLikeOrTerminalNameLike(new StringBufferWrapper().append("%").append(name).append("%").toString(), page);
		}
