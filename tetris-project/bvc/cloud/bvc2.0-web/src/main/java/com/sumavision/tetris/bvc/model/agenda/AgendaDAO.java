...
public interface AgendaDAO extends BaseDAO<AgendaPO>{
	...
	@Query(value = "select *.id from TETRIS_BVC_MODEL_AGENDA agenda left join TETRIS_BVC_BUSINESS_RUNNING_AGENDA m on agenda.id = m.agenda_id where m.group_id = ?1", nativeQuery = true)
