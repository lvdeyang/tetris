...
import com.sumavision.tetris.bvc.model.dao.TerminalBundleDAO;
...
//			pageTaskService.addAndRemoveTasks(pageInfo, newTasks, null);
	...
	public List<CommonForwardPO> obtainCommonForwards(AgendaPO agenda, Long groupId){
		...
		for(AgendaForwardPO agendaForward : agendaForwards){
			...
			Long sourceId = agendaForward.getSourceId();
			switch (sourceType) {
			case ROLE:
				RolePO sourceRole = roleDao.findOne(sourceId);
			...
			Long destinationId = agendaForward.getDestinationId();
			List<CommonForwardPO> forwards = new ArrayList<CommonForwardPO>();
			switch (destinationType) {
			case ROLE:
				RolePO dstRole = roleDao.findOne(destinationId);
