...
@RepositoryDefinition(domainClass = CommonMemberPO.class, idClass = long.class)
public interface CommonMemberDAO extends MetBaseDAO<CommonMemberPO>{
...
//	public Page<DeviceGroupMemberDTO> findGroupRecordMembersByGroupId(Long groupId, Pageable page);
	...
	public List<CommonMemberPO> findByRoleIdIn(Set<Long> roleIds);
...
//	public List<Long> findGroupIdByBundleId(String bundleId);
	...
	public List<CommonMemberPO> findByBundleId(String bundleId);

	public CommonMemberPO findByGroupIdAndBundleId(Long groupId, String bundleId);	

	//进行中的会议（不含监控室）的成员总数
	@Query(value="SELECT COUNT(*) FROM bvc_common_member member INNER JOIN bvc_common_group _group ON member.group_id=_group.id WHERE (_group.status='START' AND _group.type!='MONITOR')", nativeQuery=true)
