...
public class AgendaForwardQuery {
	 ...
	 */
	public Set<String> queryBusinessInfoTypes() throws Exception{
	...
	public List<AgendaForwardVO> packForwards(List<AgendaForwardPO> forwardEntities) throws Exception{
		...
		if(forwardEntities!=null && forwardEntities.size()>0){
			...
			for(AgendaForwardPO forwardEntity:forwardEntities){
				if(forwardEntity.getSourceType() != null){
				...
				if(forwardEntity.getAudioSourceType() != null){
					if(forwardEntity.getAudioSourceType().equals(AgendaSourceType.ROLE)){
						roleIds.add(Long.valueOf(forwardEntity.getAudioSourceId()));
					}else if(forwardEntity.getAudioSourceType().equals(AgendaSourceType.ROLE_CHANNEL)){
						roleChannelIds.add(Long.valueOf(forwardEntity.getAudioSourceId()));
					}else if(forwardEntity.getAudioSourceType().equals(AgendaSourceType.COMBINE_VIDEO)){
						combineVideoIds.add(Long.valueOf(forwardEntity.getAudioSourceId()));
					}else if(forwardEntity.getAudioSourceType().equals(AgendaSourceType.COMBINE_AUDIO)){
						combineAudioIds.add(Long.valueOf(forwardEntity.getAudioSourceId()));
					}else if(forwardEntity.getAudioSourceType().equals(AgendaSourceType.BUNDLE)){
						bundleIds.add(forwardEntity.getAudioSourceId());
					}else if(forwardEntity.getAudioSourceType().equals(AgendaSourceType.CHANNEL)){
						bundleIds.add(forwardEntity.getAudioSourceId().split("-")[0]);
					}
				}
			...
			for(AgendaForwardVO forward:forwards){
				...
				if(forward.getAudioSourceType() != null){
					if(forward.getAudioSourceType().equals(AgendaSourceType.ROLE.toString())){
						if(roles!=null && roles.size()>0){
							for(RolePO role:roles){
								if(role.getId().toString().equals(forward.getAudioSourceId())){
									forward.setAudioSourceName(role.getName());
									break;
								}
							}
						}
					}else if(forward.getAudioSourceType().equals(AgendaSourceType.ROLE_CHANNEL.toString())){
						if(roleChannels!=null && roleChannels.size()>0){
							for(RoleChannelPO roleChannel:roleChannels){
								if(roleChannel.getId().toString().equals(forward.getAudioSourceId())){
									RolePO targetRole = null;
									for(RolePO role:roles){
										if(role.getId().equals(roleChannel.getRoleId())){
											targetRole = role;
									...
									forward.setAudioSourceName(new StringBufferWrapper().append(targetRole.getName()).append("-").append(roleChannel.getName()).toString());
									break;
								}
							}
						}
					}/*else if(forward.getAudioSourceType().equals(AgendaSourceType.COMBINE_VIDEO.toString())){
						if(combineVideos!=null && combineVideos.size()>0){
							for(CombineVideoPO combineVideo:combineVideos){
								if(combineVideo.getId().toString().equals(forward.getAudioSourceId())){
									forward.setAudioSourceName(combineVideo.getName());
									break;
								}
							}
						}
					}*/else if(forward.getAudioSourceType().equals(AgendaSourceType.COMBINE_AUDIO.toString())){
						if(combineAudios!=null && combineAudios.size()>0){
							for(CombineAudioPO combineAudio:combineAudios){
								if(combineAudio.getId().toString().equals(forward.getAudioSourceId())){
									forward.setAudioSourceName(combineAudio.getName());
									break;
								}
							}
						}
					}else if(forward.getAudioSourceType().equals(AgendaSourceType.BUNDLE.toString())){
						if(bundles!=null && bundles.size()>0){
							for(BundlePO bundle:bundles){
								if(bundle.getBundleId().equals(forward.getAudioSourceId())){
									forward.setAudioSourceName(bundle.getBundleName());
									break;
								}
							}
						}
					}else if(forward.getAudioSourceType().equals(AgendaSourceType.CHANNEL.toString())){
						if(bundleIds!=null && bundles.size()>0){
							for(BundlePO bundle:bundles){
								if(forward.getAudioSourceId().startsWith(bundle.getBundleId())){
									forward.setAudioSourceName(forward.getAudioSourceId().replace(bundle.getBundleId(), bundle.getBundleName()));
									break;
								}
							}
						}
