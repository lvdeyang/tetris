...
import com.sumavision.bvc.common.group.dao.CommonCombineAudioSrcDao;
...
@Service
public class CommonAudioServiceImpl {
	...
	@Autowired
	private CommonCombineAudioSrcDao commonCombineAudioSrcDao;
	...
	public LogicBO setGroupAudio(CommonGroupPO group, Collection<Long> totalVoicedIds) throws Exception{
	...
	public LogicBO setGroupAudio(
		...
		//只需要已连接的成员id
		Set<Long> connectTotalVoicedIds = commonQueryUtil.filterConnectIds(group, totalVoicedIds);
		
		CommonAvtplPO avtpl = group.getAvtpl();
		CommonAvtplGearsPO currentGear = commonQueryUtil.queryCurrentGear(group);
		List<CommonMemberPO> members = commonQueryUtil.queryEffectiveMembers(group);
		List<Long> effectiveMemberIds = commonQueryUtil.queryEffectiveMemberIds(group);
		List<Long> totalMemberIds = commonQueryUtil.queryAllMemberIds(group);
		Set<CommonChannelForwardPO> forwards = group.getForwards();
		Set<CommonCombineAudioPO> audios = group.getCombineAudios();
		
		//生成旧的混音以
		List<CombineAudioBO> oldAudios = CombineAudioBO.generateFromCombineAudios_Common(audios);
		...
		if(needAddAudios.size() > 0){
			for(CombineAudioBO addAudio:needAddAudios){
				CommonCombineAudioPO newAudio = new CommonCombineAudioPO();
				//记录uuid
				addAudio.setUuid(newAudio.getUuid());
				newAudio.setSrcs(new HashSet<CommonCombineAudioSrcPO>());
				List<Long> srcs = addAudio.getSrcs();
				for(Long src:srcs){
					CommonMemberChannelPO encodeAudioChannel = commonQueryUtil.queryEncodeAudioChannel(group, src);
					if(null != encodeAudioChannel){
						CommonCombineAudioSrcPO audioSrc = new CommonCombineAudioSrcPO().set(encodeAudioChannel);
						//建立关联
				...
				if(group.getCombineAudios()==null) group.setCombineAudios(new HashSet<CommonCombineAudioPO>());
				group.getCombineAudios().add(newAudio);
			}
		}
		
		//将要删除的混音源
		Set<CommonCombineAudioSrcPO> willDeleteSrcs = new HashSet<CommonCombineAudioSrcPO>();
		...
		if(needCoverAudios.size() > 0){
			...
			for(CombineAudioBO scopeNewAudio:scopeNewAudios){
				...
				CommonCombineAudioPO scopeEntityAudio = commonQueryUtil.queryCombineAudio(group, scopeOldAudio.getUuid());
				Set<CommonCombineAudioSrcPO> scopeEntityAudioSrcs = scopeEntityAudio.getSrcs();
				//解关联
				for(CommonCombineAudioSrcPO scopeEntityAudioSrc:scopeEntityAudioSrcs){
				...
				for(Long scopeNewVoicedId:scopeNewVoicedIds){
					CommonMemberChannelPO encodeAudioChannel = commonQueryUtil.queryEncodeAudioChannel(group, scopeNewVoicedId);
					if(null != encodeAudioChannel){
						CommonCombineAudioSrcPO audioSrc = new CommonCombineAudioSrcPO().set(encodeAudioChannel);
						//建立关联
		...
		if(needEmptyAudios.size() > 0){
			for(CombineAudioBO scopeEmptyAudio:needEmptyAudios){
				CommonCombineAudioPO scopeEntityAudio = commonQueryUtil.queryCombineAudio(group, scopeEmptyAudio.getUuid());
				Set<CommonCombineAudioSrcPO> scopeEntityAudioSrcs = scopeEntityAudio.getSrcs();
				//解关联
				for(CommonCombineAudioSrcPO scopeEntityAudioSrc:scopeEntityAudioSrcs){
					scopeEntityAudioSrc.setCombineAudio(null);
		...
		if(willDeleteSrcs.size() > 0) commonCombineAudioSrcDao.deleteInBatch(willDeleteSrcs);
		
		//需要删除的音频转发
		List<CommonChannelForwardPO> willDeleteForward = new ArrayList<CommonChannelForwardPO>();
		
		//维护数据库转发数据
		for(Long memberId:totalMemberIds){
			CommonMemberChannelPO decodeAudioChannel = commonQueryUtil.queryDecodeAudioChannel(group, memberId);
			if(decodeAudioChannel == null) continue;
			CommonChannelForwardPO audioForward = commonQueryUtil.queryAudioForward(group, memberId);
				...
				for(CombineAudioBO scopeNochangeAudio:scopeNochangeAudios){
					if(scopeNochangeAudio.isSuitableForMember(memberId)){
						if("combineJv230".equals(decodeAudioChannel.getBundleType())){
							//TODO 处理拼接屏音频转发
//							CombineJv230PO combineJv230 = commonQueryUtil.queryCombineJv230(group, decodeAudioChannel.getMember().getId());
//							Jv230ChannelPO jv230Channel = commonQueryUtil.queryJv230ChannelPO(combineJv230);							
//							if(scopeNochangeAudio.getSrcs().size() == 1){
//								CommonMemberChannelPO encodeAudioChannel = commonQueryUtil.queryEncodeAudioChannel(group, scopeNochangeAudio.getSrcs().iterator().next());
...
//							}						
							...
							if(audioForward == null){
								audioForward = new CommonChannelForwardPO();
...
//							}	
							...
							if(needAddAudio.getSrcs().size() == 1){
								//这个地方走转发
								CommonMemberChannelPO encodeAudioChannel = commonQueryUtil.queryEncodeAudioChannel(group, needAddAudio.getSrcs().iterator().next());
								audioForward.generateAudioForward(encodeAudioChannel, decodeAudioChannel);
							}else {
								//这个地方走混音
...
//							}	
							...
							if(scopNeedCoverNewAudio.getSrcs().size() == 1){
								//这个地方走转发
								CommonMemberChannelPO encodeAudioChannel = commonQueryUtil.queryEncodeAudioChannel(group, scopNeedCoverNewAudio.getSrcs().iterator().next());
								audioForward.generateAudioForward(encodeAudioChannel, decodeAudioChannel);
							}else{
								//这个地方走混音
		...
		for(CommonMemberPO member:group.getMembers()){
		...
		if(needAddAudios!=null && needAddAudios.size()>0){
			...
			List<CommonCombineAudioPO> scopeNeedAddAudioEntities = commonQueryUtil.queryCombineAudio(group, audioAddUuids);
			logic.setCombineAudioSet_Common(scopeNeedAddAudioEntities, codec);
		}
		...
		if(audioUpdateUuids.size() > 0){
			List<CommonCombineAudioPO> scopeNeedUpdateAudioEntities = commonQueryUtil.queryCombineAudio(group, audioUpdateUuids);
			logic.setCombineAudioUpdate_Common(scopeNeedUpdateAudioEntities, codec);
		}
		
		//清除音频转发
		if(willDeleteForward.size() > 0) logic.deleteForward_Common(willDeleteForward, codec);
		
		//建立转发
		if(doForward) logic.setForward_Common(forwards, codec);
