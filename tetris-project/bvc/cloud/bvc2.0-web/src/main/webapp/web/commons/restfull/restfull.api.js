...
define([
    ...
    //成功处理
    var handleSuccess = function(data, _success, _catch, $loading, opt){
        ...
        if(_status === ajax.TIMEOUT){
            ...
            //服务器异常
            conf.messenger.error('服务器异常：'+data.message, status);
            //window.location.hash = baseError + '/' + _status + '/' + encodeURI(data.message);
        }else if(_status !== ajax.SUCCESS){
            //服务器异常
            conf.messenger.error('未知异常：'+data.message, status);
            console.log(data);
            console.log(opt);
            console.log('***********我倒要看看这个nan错误是什么鬼**************');
        ...
        if(typeof _success==='function' && (_status===ajax.SUCCESS || _catch[_status]===true)){
            var externalHttpResponse = false;
            for(var i in data){
                if(i!=='status' && i!=='message' && i!=='data'){
                    externalHttpResponse = true;
                    break;
                }
            }
            if(externalHttpResponse){
    ...
    var ajax = {
            ...
            $.ajax = function(opt){
                ...
                var _opt = $.extend(opt, {
                    error:function(XMLHttpRequest, textStatus, errorThrown) {
                        ...
                        if(XMLHttpRequest.status===401 && ajax._conf.login){
                            //重定向到登录
                            if(ajax._conf.redirectLogin){
                                window.location.href = ajax._conf.login;
                                console.log(XMLHttpRequest);
                                console.log(401);
                                return;
                            }else{
                                handleSuccess({status:ajax.FORBIDDEN, message:'服务端返回401'}, _success, _catch, $loading, _opt);
                            }
                        }
                ...
                if(!conf.debug && opt.dataType!=='jsonp'){
                    ...
                    _opt.success = function(data){
                        ...
                        handleSuccess(data, _success, _catch, $loading, _opt);
                    };
                ...
                if(!_opt.contentType) _opt.contentType = 'application/x-www-form-urlencoded; charset=UTF-8';
        ...
        //get
        get:function(uri, data, callback, contentType, catchCodeArr, $loading, dataType, async){
        ...
        //getExtral
        getExtral:function(uri, data, callback, contentType, catchCodeArr, $loading, dataType){
        ...
        //post
        post:function(uri, data, callback, contentType, catchCodeArr, $loading, dataType, async){
        ...
        //update
        update:function(uri, data, callback, contentType, catchCodeArr, $loading, dataType, async){
        ...
        //remove
        remove:function(uri, data, callback, contentType, catchCodeArr, $loading, dataType, async){
