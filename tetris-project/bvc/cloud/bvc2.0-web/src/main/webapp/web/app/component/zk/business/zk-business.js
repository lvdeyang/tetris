define([
  'text!' + window.APPPATH + 'component/zk/business/zk-business.html',
  'restfull',
  'jquery',
  'vue',
  'config',
  'element-ui',
  'css!' + window.APPPATH + 'component/zk/business/zk-business.css'
], function (tpl, ajax, $, Vue, config) {

  //组件名称
  var pluginName = 'zk-business';
  ...
  Vue.component(pluginName, {
    ...
    watch: {
      currentTab: function () {
        ...
        if (self.currentTab == 0) {
          //组织结构
          if (self.tree.institution.data.length === 0) {
          ...
          //文件资源
          if (self.tree.file.data.length === 0) {
            self.refreshFile();
          }
          ...
          //指挥列表
          if (self.tree.command.data.length === 0) {
            self.refreshCommand();
          }
    ...
    methods: {
      ...
      onFileCheckChange: function (data) {
        var self = this;
        for (var i = 0; i < self.tree.file.select.length; i++) {
          if (self.tree.file.select[i] === data) {
            self.tree.file.select.splice(i, 1);
            return;
          }
        }
        self.tree.file.select.push(data);
      },
      //录制回放的复选框的change事件
      onRecordCheckChange: function (data) {
        ...
        //判断选中的中的level是否都相同
        self.buttons.batchVod = self.isSameLevel(self.tree.record.select);
      },
      ...
      onCommandCheckChange: function (data) {
        ...
        for (var i = 0; i < self.tree.command.select.length; i++) {
          if (self.tree.command.select[i] === data) {
            self.tree.command.select.splice(i, 1);
            return;
          }
        }
        self.tree.command.select.push(data);
        var disable = false;
        for (var i = 0; i < self.tree.command.select.length; i++) {
          var creator = $.parseJSON(self.tree.command.select[i].param).creator;
      ...
      refreshFile: function () {
        var self = this;
        self.tree.file.data.splice(0, self.tree.file.data.length);
        ajax.post('/monitor/vod/query/resource/tree', null, function (data) {
          if (data && data.length > 0) {
            for (var i = 0; i < data.length; i++) {
              self.tree.file.data.push(data[i]);
            }
          }
        });
      },
                ...
                for (var j = 0; j < data.groups[i].records.length; j++) {
                  ...
                  if (data.groups[i].records[j].fragments.length) {
                    for (var k = 0; k < data.groups[i].records[j].fragments.length; k++) {
                      ...
                      data.groups[i].records[j].fragments[k].type = 'RECORD_FRAGMENT_PLAYBACK';
      ...
      refreshCommand: function () {
        var self = this;
        self.tree.command.data.splice(0, self.tree.command.data.length);
        ajax.post('/command/query/list', null, function (data) {
          if (data && data.length > 0) {
            ...
            self.tree.command.data.push(joinCommand);
            self.tree.command.data.push(myCommand);
          }
        });
      },
      ...
      handleContextMenuShow: function (e, data, type) {
        ...
        self.contextMenu.left = e.clientX + 'px';
        ...
        } else if (data.type === 'RECORD_FRAGMENT_PLAYBACK') {
          self.contextMenu.call = false;
          self.contextMenu.vod = false;
          self.contextMenu.intercom = false;
          self.contextMenu.dedicatedCommand = false;
          self.contextMenu.enterCommand = false;
          if (self.contextMenu.currentNode.level == 3) {
            self.contextMenu.removeRecord = false;
          } else {
            self.contextMenu.removeRecord = false;
          }
      ...
      enterCommandBatch: function () {
        ...
        if (self.tree.command.select.length <= 0) {
          self.qt.alert('消息提示', '您还没有选择任何指挥组');
        }
      ...
      addCommandMember: function () {
        ...
        self.qt.window('/router/zk/business/add/member', {
          id: self.group.current.id,
      ...
      cooperativeCommand: function () {
        ...
        self.qt.window('/router/zk/business/cooperation', {
          id: self.group.current.id,
          type: self.group.current.type
        }, {
          width: 500,
      ...
      commandForward: function () {
        ...
        self.qt.window('/router/zk/business/forward', {
          id: self.group.current.id
        }, {
          width: 1024,
          height: 768,
      ...
      silenceUp: function () {
        ...
        if (self.buttons.silenceUpStart) {
          ajax.post('/command/basic/silence/up/start', {
            id: self.group.current.id
          }, function () {
            self.buttons.silenceUpStart = false;
            self.qt.alert('提示消息', '操作成功');
      ...
      silenceDown: function () {
        ...
        if (self.buttons.silenceDownStart) {
          ajax.post('/command/basic/silence/down/start', {
            id: self.group.current.id
          }, function () {
            self.buttons.silenceDownStart = false;
            self.qt.alert('提示消息', '操作成功');
      ...
      sendMessage: function () {
        ...
        self.qt.window('/router/zk/business/command/message', {
          id: self.group.current.id
        }, {
          width: 1024,
          height: 768,
      ...
      callUserBatch: function () {
        var self = this;
        if (self.tree.institution.select.length <= 0) {
        ...
        ajax.post('/command/call/user/start/batch', {
      ...
      voiceIntercom: function () {
        var self = this;
        var userId = self.contextMenu.currentNode.id;
        ajax.post('/command/voice/intercom/start', {
          userId: userId
        }, function (data) {
      ...
      voiceIntercomBatch: function () {
        ...
        ajax.post('/command/voice/intercom/start/batch', {
          userIds: $.toJSON(userIds)
        }, function (data) {
          self.qt.invoke('voiceIntercoms', $.toJSON(data));
        });
      ...
      vod: function () {
        ...
        if (self.contextMenu.currentNode.type === 'USER') {
          ...
          if (self.contextMenu.scope === 'command' && self.group.current.creator == self.user.id) {
            ...
            ajax.post('/command/vod/user/start', {
              userId: userId
            }, function (data) {
          ...
          ajax.post('/command/vod/resource/file/start', {
          ...
          ajax.post('/command/record/start/playback', {
            recordId: recordId
          }, function (data) {
      ...
      vodBatch: function () {
        ...
        if (self.currentTab == 0) {
          ...
          ajax.post('/command/vod/user/start/batch', {
            userIds: $.toJSON(userIds)
          }, function (data) {
            self.qt.invoke('vodUsers', $.toJSON(data));
          });
          ...
          var deviceIds = [];
          for (var i = 0; i < self.tree.device.select.length; i++) {
            deviceIds.push(self.tree.device.select[i].id);
          }
          ajax.post('/command/vod/device/start/batch', {
            deviceIds: $.toJSON(deviceIds)
          }, function (data) {
            self.qt.invoke('vodDevices', $.toJSON(data));
          });
        } else if (self.currentTab == 2) {
          //点播文件
          if (self.tree.file.select.length <= 0) {
            self.qt.alert('消息提示', '您还没有选中文件');
            return;
          }
          var resourceFileIds = [];
          for (var i = 0; i < self.tree.file.select.length; i++) {
            resourceFileIds.push(self.tree.file.select[i].id);
          }
          ajax.post('/command/vod/resource/file/start/batch', {
            resourceFileIds: $.toJSON(resourceFileIds)
          }, function (data) {
            self.qt.invoke('vodResourceFiles', $.toJSON(data));
          });
        } else if (self.currentTab == 3) {
          ...
          for (var i = 0; i < self.tree.record.select.length; i++) {
            recordIds.push(self.tree.record.select[i].id);
          }
      ...
      secretCommand: function () {
        var self = this;
        var userId = self.contextMenu.currentNode.id;
        ajax.post('/command/secret/start', {
          userId: userId
        }, function (data) {
    ...
    mounted: function () {
      var self = this;
      $(document).on('mousedown.business.contextMenu', function (e) {
        var $target = $(e.target);
        if (!($target.is('.context-menu') || $target.closest('.context-menu')[0])) {
          self.handleContextMenuClose();
        }
      });
      self.qt = new QtContext('business', function () {
        ...
        self.qt.on('mousedown', function (e) {
          if (e.scope !== 'business') {
            self.handleContextMenuClose();
          }
        });
        ...
        self.qt.on('callUserMessage', function (e) {
          ...
          if (self.settings.responseMode === 'auto') {
            setTimeout(function () {
              ajax.post('/command/call/user/accept', {
            ...
            self.qt.confirm('业务提示', businessInfo, '拒绝', '同意', function () {
              ajax.post('/command/call/user/refuse', {
                businessId: businessId
              }, null);
            }, function () {
              ajax.post('/command/call/user/accept', {
        ...
        self.qt.on('callUserRefuseMessage', function (e) {
          ...
          self.qt.invoke('callUserStop', $.toJSON({
            serial: serial
          }));
        });
        //websocket 停止呼叫
        ...
        self.qt.on('voiceIntercomMessage', function (e) {
          ...
          if (self.settings.responseMode === 'auto') {
            setTimeout(function () {
              ajax.post('/command/voice/intercom/accept', {
                businessId: businessId
              }, function (data) {
                self.qt.invoke('voiceIntercoms', $.toJSON([data]));
              });
            ...
            self.qt.confirm('业务提示', businessInfo, '拒绝', '同意', function () {
              ajax.post('/command/voice/intercom/refuse', {
                businessId: businessId
              }, null);
            }, function () {
              ajax.post('/command/voice/intercom/accept', {
        ...
        self.qt.on('voiceIntercomRefuseMessage', function (e) {
          ...
          var serial = callUserInfo.serial;
          self.qt.alert('业务提示', businessInfo);
          self.qt.invoke('voiceIntercomStop', $.toJSON({
            serial: serial
          }));
        });
        //websocket 停止语音对讲
        ...
        self.qt.on('cooperationGrant', function (e) {
          ...
          if (self.settings.responseMode === 'auto') {
            setTimeout(function () {
              ajax.post('/command/cooperation/agree', {
                id: e.businessId
              }, null);
            }, 800);
          } else {
            self.qt.confirm('业务提示', e.businessInfo, '拒绝', '同意', function () {
              ajax.post('/command/cooperation/refuse', {
                id: e.businessId
              }, null);
            }, function () {
              ajax.post('/command/cooperation/agree', {
        ...
        self.qt.on('commandStart', function (e) {
          ...
          self.qt.confirm('业务提示', e.businessInfo, '拒绝', '同意', function () {
            //拒绝进入指挥
            ajax.post('/command/basic/refuse', {
              id: e.businessId
            }, function () {});
          }, function () {
            //同意进入指挥
        ...
        self.qt.on('commandMessageStop', function (e) {
          e = e.params;
          self.qt.alert('业务提示', e.businessInfo);
          self.qt.invoke('commandMessageStop', $.toJSON({
        ...
        self.qt.on('secretStart', function (e) {
          ...
          if (self.settings.responseMode === 'auto') {
            setTimeout(function () {
              ajax.post('/command/secret/accept', {
                id: e.businessId
              }, function (data) {
                self.qt.invoke('secretStart', $.toJSON([data]));
              });
            ...
            self.qt.confirm("业务提示", e.businessInfo, '拒绝', '同意', function () {
              ajax.post('/command/secret/refuse', {
                id: e.businessId
              }, function () {});
            }, function () {
              ajax.post('/command/secret/accept', {
        ...
        self.qt.on('secretRefuse', function (e) {
          e = e.params;
          self.qt.alert('业务提示', e.businessInfo);
          self.qt.invoke('secretStop', $.toJSON([{
            serial: e.serial
          }]));
        });
