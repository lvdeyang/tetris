define([
    ...
    //检查当前有没有未保存的行
    var __hasUnsavedRow = function(rows){
        if(!rows || rows.length<=0) return false;
        for(var i=0; i<rows.length; i++){
            if(rows[i].__rowStatus===__rowStatusCreate ||
                rows[i].__rowStatus===__rowStatusEdit){
                return true;
            }
        }
        return false;
    ...
    //查找当前没有保存的行
    var __findUnsavedRow = function(rows){
    ...
    //设置当前行
    var __setCurrentRow = function(rows, currentRow){
        if(!rows || rows.length<=0) return;
        for(var i=0; i<rows.length; i++){
            if(rows[i] === currentRow){
                rows[i].__isCurrent = true;
            }else{
                rows[i].__isCurrent = false;
            }
        }
    };
    ...
    Vue.component(bvc2SystemTableBase, {
        ...
        computed:{
            optionsWidth:function(){
                ...
                var length = Math.max(length1, length2, length3)*50;
                if(length > 150){
                	length = Math.max(length1, length2, length3)*35;
                }
                return length;
            }
        },
        methods: {
            ...
            handleRowClick:function(row, e, column){
                ...
                var lock = __hasUnsavedRow(table_instance.rows);
                if(!lock){
                    table_instance.$refs.multipleTable.setCurrentRow(row);
                    __setCurrentRow(table_instance.rows, row);
                    __emit(table_instance, __rowClick, row, e);
                }else{
                    var unsavedRow = __findUnsavedRow(table_instance.rows);
                    table_instance.$refs.multipleTable.setCurrentRow(__findUnsavedRow(table_instance.rows));
                    __setCurrentRow(table_instance.rows, unsavedRow);
                }
            },
            ...
            create:function(e){
                var table_instance = this;
                if(__hasUnsavedRow(table_instance.rows)){
                    table_instance.$message({
                        message: '当前有未保存的数据，请先保存！',
                        type: 'warning'
                    });
                    return;
                }
                ...
                var done = function(data){
                    ...
                    //数据行选中
                    table_instance.$refs.multipleTable.setCurrentRow(newRow);

                    //数据行发生变化
                    __emit(table_instance, __rowChanged, table_instance.rows, tableSelector);

                    //__emit(table_instance, __rowCreate + __suffix, data , e);
                };

                __emit(table_instance, __rowCreate + __prefix, null , e, done );
            ...
            rowEdit:function(e, scope, eName){
                ...
                //数据行选中
                table_instance.$refs.multipleTable.setCurrentRow(scope.row);
            ...
            handleDialogOk:function(e){
                ...
                if(typeof column.change === 'function'){
                    column.change(row, column, prop, table_instance);
                }else{
