define([
  ...
  Vue.component(pluginName, {
    ...
    methods: {
      ...
      enterCommand: function (commandIds) {
        ...
        ajax.post('/command/basic/enter', {
          ...
          if (data && data.length > 0) {
            ...
            self.qt.linkedWebview('leaderDialogCreateGroup', {
              id: 'close'
            });
            self.qt.linkedWebview('leaderDialogAddGroup', {
              id: 'close'
            });
            self.qt.linkedWebview('leaderDialogPrepareGroup', {
              id: 'close'
            });
          }
        });
      ...
      currentGroupChange: function (v) {
        ...
        if (currentGroup.type === 'command') {
          ...
          ajax.post('/command/basic/query/members/list', {
            id: currentGroup.id
      ...
      onCommandUserCheckChange: function (data) {
        ...
        for (var i = 0; i < self.group.current.select.length; i++) {
          if (self.group.current.select[i].id === data.id) {
      ...
      removeCommandMember: function () {
        ...
        self.qt.confirm('提示信息', '当前强退' + self.group.current.select.length + '个成员', '取消', '确定', null, function () {
          ...
          ajax.post('/command/basic/remove/members', {
            ...
            if (data && data.length > 0) {
              self.qt.linkedWebview('historyMessage', {
                id: 'commandMemberDeleteProxy',
                params: data
              });
            }
          }, null, [403]);
      ...
      //取消协同
      cancelCooperativeCommand: function () {
        var self = this;
      ...
      commandForward: function () {
        ...
        self.qt.window('/router/zk/leader/forward', {
          id: self.group.current.id
        }, {
          width: 1024,
          height: 768,
      ...
      sendMessage: function () {
        ...
        self.qt.window('/router/zk/leader/send/message', {
          id: self.group.current.id
        }, {
          width: 1224,
          height: 768
          ...
          ajax.post('/command/basic/stop', {
            ...
            if (self.group.current.creator != self.user.id) {
              ...
              if (self.group.entered.length > 0) {
                self.group.current = self.group.entered[0];
                self.group.currentId = self.group.entered[0].id;
                if (self.group.currentId) self.currentGroupChange(self.group.currentId);
              } else {
                self.group.current = '';
                self.group.currentId = '';
              }
      ...
      // 接口改了，需要判断是否重名的情况，这的代码还没改，先注释了，要改的话，参考新建会议的代码。yan -2020-03-04
      //给会议起名字的添加事件
    ...
    watch: {
      filterCommandUserText: function (val) {
        this.$refs.commandUserTree.filter(val);
      },
      currentTab: function () {
        var self = this;
    ...
    mounted: function () {
      ...
      self.qt = new QtContext('rightBar', function () {
        ...
        //给scrollTab 设置初始高度
        self.contentStyleObj.height = Math.abs((parseInt(window.getComputedStyle(self.$refs.firstMenu).height) - parseInt(window.getComputedStyle(self.$refs.footer).height))) + 'px';
        ...
        self.qt.on('linkEnterCommand', function (e) {
          if (self.group.current) {
            console.log('重复的会议:'+self.group.name);
            self.qt.error('当前正在参加会议，请先退出！');
          } else {
            self.enterCommand([e.params]);
          }
        });

        // //websocket 停止会议  成员监听到主席停止会议，关闭成员播放器
        self.qt.on('usercommandStop', function (e) {
          ...
          if ((self.group.currentId == e.businessId) || (self.group.current && self.group.current.id === e.businessId)) {
            if (self.group.entered.length > 0) {
              self.group.current = self.group.entered[0];
              self.group.currentId = self.group.entered[0].id;
              self.currentGroupChange(self.group.currentId);
