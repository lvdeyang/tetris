        ...
        methods: {
            ...
            enterCommand: function (commandIds) {
                ...
                ajax.post('/command/basic/enter', {ids: $.toJSON(commandIds)}, function (data) {
                    if (data && data.length > 0) {
                        ...
                        self.qt.linkedWebview('leaderDialogCreateGroup', {id:'close'});
                        self.qt.linkedWebview('leaderDialogAddGroup', {id:'close'});
                        self.qt.linkedWebview('leaderDialogPrepareGroup', {id:'close'});
                    }
                });
            },
            //获取会议工作台树形菜单的数据
            ...
            currentGroupChange: function (v) {
                ...
                if (currentGroup.type === 'command') {
                    ...
                    ajax.post('/command/query/find/command/cooperation/list', {id: currentGroup.id}, function (data, status) {
            ...
            commandForward: function () {
                ...
                self.qt.window('/router/zk/leader/forward', {id: self.group.current.id}, {
                    width: 1024,
                    height: 768,
                    title: '会议转发'
                });
            ...
            exitCurrentCommandAndCloseWindow: function () {
                ...
                if(!self.group.current){
                    self.qt.linkedWebview('header', {id:'closeWindow'});
                }else{
                    if (self.user.id == self.group.current.creator) {
                        ajax.post('/command/basic/stop', {
                            id: self.group.current.id
