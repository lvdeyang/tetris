    ...
    methods: {
      ...
      handleSendMessage: function () {
        ...
        ajax.post('/command/message/query/message', {
          ...
          currentPage: self.dialog.messageList.page.currentPage
        }, function (data) {
          var rows = data.rows;
          if (rows && rows.length > 0) {
            ...
            if (count > 3) {
              self.qt.info('当前有3通知，为了防止屏幕显示不全，请先关闭一些通知，再发布新的通知');
              return;
            }
          }
      ...
      deleteMessage: function (scope) {
        var self = this;
        var row = scope.row;
        ajax.post('/command/message/remove', {
          messageIds: $.toJSON([row.id])
        }, function () {
          for (var i = 0; i < self.dialog.messageList.data.length; i++) {
      ...
      handleMessageCurrentChange: function (currentPage) {
        var self = this;
        this.dialog.messageList.page.currentPage = currentPage;
      ...
      loadMessage: function (currentPage) {
        ...
        ajax.post('/command/message/query/message', {
          ...
          self.dialog.messageList.data.splice(0, self.dialog.messageList.data.length);
          self.dialog.messageList.data=data.rows;
      ...
      handleStyleTemplatesCurrentChange: function (currentPage) {
        var self = this;
        this.dialog.styleTemplates.page.currentPage = currentPage;
