...
+function(){
    ...
    var publics = {
        ...
        create:function(options){
            ...
            if(options.theme) $table.addClass(options.theme);
        ...
        getData:function(){
            var $cell = $(this);
            return $cell.data('business-cache');
        },
            ...
            for(var i=0; i<$trs.length; i++){
                ...
                for(var j=0; j<$tds.length; j++){
                    ...
                    if(!$td.is(':hidden')){
                        ...
                        var l = {
                            serialNum:serialNum,
                            x:cell.relative.x + '/' + options.cell.column,
                            y:cell.relative.y + '/' + options.cell.row,
        ...
        //生成模板
        generateTpl:function(withoutData){
            var $container = $(this);
            var $table = $container.find('.layout-auto');
            var options = $table.data('options');
            var layout = publics.generateLayout.apply(this, [withoutData]);
            return {
                basic:{
        ...
        getSerialNum:function(){
            var $target = $(this);
            var $table = $target.closest('.layout-auto');
            var $trs = $table.find('tbody tr');
            var serialNum = 1;
            for(var i=0; i<$trs.length; i++){
                var $tr = $($trs[i]);
    ...
    var privates = {
        ...
        generateHtml:function(cell){
            ...
            for(var i=0; i<column; i++){
                if(i === column-1){
                    html += '<td style="width:'+lastWidth+'%; padding:0;"></td>';
                }else{
                    html += '<td style="width:'+perWidth+'%; padding:0;"></td>';
                }
            }
            ...
            for(var i=0; i<row; i++){
                ...
                for(var j=0; j<column; j++){
                    if(i === column-1){
                        html += '<td class="cell '+_style+'" ondragstart="return false;">'+(_html?_html:'&nbsp;')+'</td>';
                    }else{
                        html += '<td class="cell '+_style+'" ondragstart="return false;">'+(_html?_html:'&nbsp;')+'</td>';
                    }
                }
        ...
        initCells:function(){
            ...
            var totalWidth = $table[0].clientWidth;
            var totalHeight = $table[0].clientHeight;

            var c_per_w = parseInt(totalWidth/options.cell.column);
            var c_last_per_w = totalWidth - ((options.cell.column-1)*c_per_w);
            var c_per_h = parseInt(totalHeight/options.cell.row);
            var c_last_per_h = totalHeight - ((options.cell.row-1)*c_per_h);
            ...
            $table.find('tbody td').each(function(){
                c_counter += 1;
                if(c_counter % options.cell.column === 0){
                    r_counter += 1;
                }
                ...
                var w = (c_counter % options.cell.column === 0)?c_last_per_w:c_per_w;
                var h = (r_counter % options.cell.row === 0)?c_last_per_h:c_per_h;
    ...
    $(document).on('mousedown.layout.auto.cell', '.layout-auto .cell', function(e1){
        if(!$(e1.target).is('td.cell')) return true;
        var $begin = $(this);
