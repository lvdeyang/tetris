...
public interface MediaTxtDAO extends BaseDAO<MediaTxtPO>{
	...
	/**
	 * 获取文件夹下特定上传状态以及特定例外审核状态的文本媒资（批量）<br/>
	 * <b>作者:</b>lvdeyang<br/>
	 * <b>版本：</b>1.0<br/>
	 ...
	 */
	@Query(value = "SELECT * FROM MIMS_MEDIA_TXT WHERE FOLDER_ID IN ?1 AND ((UPLOAD_STATUS=?2 AND (REVIEW_STATUS IS NULL OR REVIEW_STATUS NOT IN ?3)) OR AUTHORID=?4)", nativeQuery = true)
	public List<MediaTxtPO> findByFolderIdInAndUploadStatusAndReviewStatusNotInOrAuthorId(Collection<Long> folderIds, String status, Collection<String> reviewStatus, String authorId);
	 ...
	 * @return List<MediaTxtPO> 视频媒资列表
	 */
	@Query(value = "SELECT * FROM MIMS_MEDIA_TXT WHERE FOLDER_ID IN ?1 AND (REVIEW_STATUS IS NULL OR REVIEW_STATUS NOT IN ?2)", nativeQuery = true)
	public List<MediaTxtPO> findByFolderIdIn(Collection<Long> folderId, Collection<String> reviewStatus);
	 ...
	 * @return List<MediaTxtPO> 查询结果
	 */
	@Query(value = "SELECT * FROM MIMS_MEDIA_TXT video WHERE IF(?1 is null, true, video.id = ?1) "
	...
	public List<MediaTxtPO> findByCondition(Long id, String name, String startTime, String endTime, String tag, List<Long> folderIds, Collection<String> reviewStatus);
