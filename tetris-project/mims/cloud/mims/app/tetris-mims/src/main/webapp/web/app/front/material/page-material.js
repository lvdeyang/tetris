 ...
 */
define([
    'text!' + window.APPPATH + 'front/material/page-material.html',
    'config',
    'context',
    'commons',
    'restfull',
    ...
    'mi-upload-dialog',
    'mi-lightbox',
    'css!' + window.APPPATH + 'front/material/page-material.css'
], function(tpl, config, context, commons, ajax, $, File, Uploader, Vue){

    var pageId = 'page-material';
    ...
    var init = function(p){
        ...
        ajax.post('/material/load/' + p.folderId, null, function(data){
            ...
            new Vue({
                ...
                methods:{
                    ...
                    //添加素材库文件夹对话框关闭初始化数据
                    addPersonalFolderDialogClosed:function(){
                        var self = this;
                        ...
                        if(row.type === 'FOLDER'){
                            self.$msgbox({
                                ...
                                beforeClose:function(action, instance, done){
                                    if(action === 'confirm'){
                                        self.removePersonalFolder(scope, instance, done);
                                    }else{
                                        done();
                                    }
                                }
                    ...
                    //删除素材库文件夹
                    removePersonalFolder:function(scope, $confirm, done){
                        var self = this;
                        var row = scope.row;
                        if($confirm) $confirm.confirmButtonLoading = true;
                        ajax.post('/folder/personal/remove/' + row.id, null, function(data, status, message){
                    ...
                    renamePersonalFolder:function(){
                        ...
                        ajax.post('/folder/personal/rename/' + row.id, {
                            folderName:folderName
                    ...
                    //重命名素材库文件夹对话框关闭初始化数据
                    renamePersonalFolderDialogClosed:function(){
                        var self = this;
                        self.dialog.renameFolder.row = null;
                        self.dialog.renameFolder.folderName = '';
                    },
                    //文件夹移动
                    movePersonalFolder:function(folder, buffer, startLoading, endLoading, close){
                        var self = this;
                        startLoading();
