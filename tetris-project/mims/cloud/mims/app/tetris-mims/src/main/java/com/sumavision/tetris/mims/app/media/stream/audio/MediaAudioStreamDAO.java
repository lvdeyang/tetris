...
public interface MediaAudioStreamDAO extends BaseDAO<MediaAudioStreamPO>{
	 ...
	 * @param Long folderId 文件夹id
	 * @param Collection<ReviewStatus> reviewStatus 例外审核状态列表
	 * @return List<MediaAudioStreamPO> 视频流媒资列表
	 */
	@Query(value = "SELECT * FROM MIMS_MEDIA_AUDIO_STREAM WHERE FOLDER_ID IN ?1 AND (REVIEW_STATUS IS NULL OR REVIEW_STATUS NOT IN ?2)", nativeQuery = true)
	public List<MediaAudioStreamPO> findByFolderIdIn(Collection<Long> folderId, Collection<String> reviewStatus);
	 ...
	 * @return List<MediaAudioStreamPO> 音频流列表
	 */
	public List<MediaAudioStreamPO> findByPreviewUrlIn(Collection<String> previewUrls);
	 ...
	 * @return List<MediaAudioStreamPO> 查询结果
	 */
	@Query(value = "SELECT * FROM MIMS_MEDIA_AUDIO_STREAM video WHERE IF(?1 is null, true, video.id = ?1) "
	...
	public List<MediaAudioStreamPO> findByCondition(Long id, String name, String startTime, String endTime, String tag, List<Long> folderIds, Collection<String> reviewStatus);
}
