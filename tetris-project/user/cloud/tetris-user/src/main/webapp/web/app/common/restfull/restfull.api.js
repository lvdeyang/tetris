...
define([
  ...
  //请求顺序数组
  // 递归
  function arrLoopFun (myAjaxArr) {
    if (myAjaxArr.length !== 0) {
      isPending = true
      myAjaxArr[0]().then(function (res) {
        res[0](res[1], res[2], res[3]) //成功后的回调
        myAjaxArr.splice(0, 1)
        arrLoopFun(myAjaxArr)
      }, function (err) {
        console.log(err)
      })
    } else {
  ...
  var ajax = {
    ...
    //按时序请求
    //post
    sequencePost: function (uri, data, callback, contentType, catchCodeArr, $loading, dataType) {
      myAjaxArr.push(function () {
        return new Promise(function (resolve, reject) {
          $.ajax({
            ...
            success: function (data, status, message) {
              resolve([callback, data, status, message])
            },
            error: function (err) {
