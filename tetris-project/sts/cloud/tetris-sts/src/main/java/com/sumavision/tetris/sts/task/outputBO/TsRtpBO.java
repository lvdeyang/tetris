...
import com.sumavision.tetris.sts.task.taskParamOutput.JsonTsRtpBO;
...
public class TsRtpBO extends CommonOutputBO {


	/**
	 * 
	 */
	private static final long serialVersionUID = -4788133133097518420L;
	...
	public OutputCommon generateOutputCommon(OutputPO outputPO, ProgramPO programPO,OutputMediaEncodeMessage outputMediaEncodeMessage) {
		...
		JsonTsRtpBO jsonTsRtpBO = new JsonTsRtpBO();
		jsonTsRtpBO.setIp(outputPO.getIp());
		jsonTsRtpBO.setPort(outputPO.getPort());
		jsonTsRtpBO.setLocal_ip(outputPO.getLocalIp());
		jsonTsRtpBO.setPcr_int(getPcrInt());
		jsonTsRtpBO.setPat_int(getPatInt());
		jsonTsRtpBO.setPmt_int(getPmtInt());
		jsonTsRtpBO.setSdt_int(getSdtInt());
		jsonTsRtpBO.setTsid_pid(getTsidPid());
		jsonTsRtpBO.setRate_ctrl(getRateCtrl()==null?"VBR":getRateCtrl());
		if (getRateCtrl()=="VBR") {
			jsonTsRtpBO.setBitrate(getMaxBitrate()==null?8000000:getMaxBitrate()*1000);
		}else {
			jsonTsRtpBO.setBitrate(getSysRate()==null?8000000:getSysRate().intValue()*1000);
		}
		jsonTsRtpBO.setBuf_init(getBuf());
		...
		jsonTsRtpBO.setProgram_array(program_array);
