  ...
  <section>
    ...
    <el-table :data="serInfoVOs" highlight-current-row v-loading="listLoading" v-if="activeTab==='serInfoTab'" @selection-change="selsChange" style="width: 100%;">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button size="small" v-if="scope.row.sourceType==='SYSTEM'" @click="handleShowEditDialog(scope.$index, scope.row)">编辑</el-button>
          <el-button size="small" v-if="scope.row.sourceType==='SYSTEM'" @click="handleDelSerInfo(scope.$index, scope.row)">删除</el-button>
          <el-button size="small" v-if="scope.row.sourceType==='SYSTEM'" @click="handleGotoGadgets(scope.$index, scope.row)">小工具</el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-form-item label="设备类型" prop="serType">
          <template>
            <el-select v-model="serInfoEditForm.serType" placeholder="请选择">
              <el-option v-for="item in serTypeOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </template>
        </el-form-item>
        <el-form-item label="协议类型" prop="serPro">
          <template>
            <el-select v-model="serInfoEditForm.serPro" placeholder="请选择">
              <el-option v-for="item in serProOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </template>
        </el-form-item>
    ...
    <el-table :data="serNodeVOs" highlight-current-row v-loading="listLoading" v-if="activeTab==='serNodeTab'" @selection-change="selsChange" style="width: 100%;">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button size="small" v-if="scope.row.sourceType==='SYSTEM'" @click="handleShowEditDialog(scope.$index, scope.row)">编辑</el-button>
          <el-button size="small" v-if="scope.row.sourceType==='SYSTEM'" @click="handleDelSerNode(scope.$index, scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="服务节点信息编辑" :visible.sync="serNodeEditFormVisible" :close-on-click-modal="false" :before-close="beforeClose">
      <el-form :model="serNodeEditForm" label-width="160px" ref="serNodeEditForm" :rules="serNodeEditFormRules">
        ...
        <el-form-item label="上级服务节点" prop="nodeFather" v-if="serNodeEditBtnVisible">
          <el-tag v-for="tag in fatherTag" :key="tag.nodeUuid" closable @close="handleFatherClose(tag)">
            {{tag.nodeName}}
          </el-tag>

          <el-button icon="el-icon-plus" class="button-new-tag-father" size="small" @click="addFatherNode">
          </el-button>
        </el-form-item>
        ...
        <el-form-item label="订阅节点" prop="nodePublish" v-if="serNodeEditBtnVisible">
          <el-select v-model="serNodeEditForm.nodePublisher" placeholder="请先选择上级服务节点">
            <el-option v-for="item in fatherTag" :key="item.nodeUuid" :label="item.nodeName" :value="item.nodeUuid">
            </el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="关联服务节点" prop="nodeRelations" v-if="serNodeEditBtnVisible">
          <el-tag v-for="tag in relationTag" :key="tag.nodeUuid" closable @close="handleClose(tag)">
            {{tag.nodeName}}
          </el-tag>

          <el-button icon="el-icon-plus" class="button-new-tag-relation" size="small" @click="addRelationNode">
          </el-button>
        </el-form-item>
    ...
    <el-dialog title="选择节点" :visible.sync="dialog.node.visible" width="800px" :before-close="handleDialogClose">
        ...
        <div style="position:absolute; left:-20px; top:-20px; right:-20px; bottom:-20px;">
          <div style="width:100%; height:100%; position:relative;">
            <div style="width:100%; position:absolute; left:0; top:0px; right:0; bottom:0; padding:0 10px; box-sizing:border-box;">
              <el-table ref="selectNodeTable" :data="dialog.node.table.data" height="100%" style="width:100%" @selection-change="handleSelectionChange">
                <el-table-column type="selection" width="55">
                </el-table-column>
                <el-table-column prop="nodeName" width="300" label="节点名称">
                </el-table-column>
                <el-table-column prop="nodeUuid" label="节点Uuid">
                </el-table-column>
              </el-table>
            </div>
          </div>
