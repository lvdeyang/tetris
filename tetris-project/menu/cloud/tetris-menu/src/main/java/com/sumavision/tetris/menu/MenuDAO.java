...
public interface MenuDAO extends BaseDAO<MenuPO>{
	 ...
	 */
	@Query(value = "select m.id, m.uuid, m.update_time, m.title, m.link, m.icon, m.style, m.is_group, m.parent_id, m.menu_id_path, m.serial, m.auto_generation from TETRIS_MENU m left join TETRIS_SYSTEM_ROLE_MENU_PERMISSION mp on m.id=mp.menu_id where mp.role_id in ?1 order by m.serial asc", nativeQuery = true)
