 ...
 */
define([
    'text!' + window.APPPATH + 'component/dialog/mims/audio/mims-audio.html',
    'restfull',
    'vue',
    'element-ui',
    'css!' + window.APPPATH + 'component/dialog/mims/audio/mims-audio.css',
], function(tpl, ajax, Vue){

    var pluginName = 'mi-audio-dialog';
    ...
    Vue.component(pluginName, {
        ...
        methods:{
            ...
            load:function(folderId){
                ...
                ajax.post('/media/audio/feign/load', {
                    folderId:folderId
            ...
            handleOkButton:function(){
                var self = this;
                self.$emit('selected-audio', self.current.previewUrl, self.__buffer, function(){
                    self.dialog.loading = true;
                }, function(){
                    self.dialog.loading = false;
                }, function(){
            ...
            audioItemClass:function(item, index){
                var c = 'audio-item';
                if(item.__select){
                    c += ' select'
                }
            ...
            audioSelect:function(audio){
                var self = this;
                for(var i=0; i<self.data.length; i++){
                    if(self.data[i].id === audio.id){
